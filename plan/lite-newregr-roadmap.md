...
`core/sql/regress/liteLegacy/newregr-inventory.tsv`. Regenerate and inspect
it with:
...
`plan/lite-legacy-regress-roadmap.md`; it does not count as newregress
...
Stateless cases receive an isolated `TRAF_LITE_STORE_DIR`. A suite whose cases
...
`make lite-newregr`, with per-suite selection and retained artifacts. It
must remain separate from `make lite-m10` and from the 161-case standard
