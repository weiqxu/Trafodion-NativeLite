...
```text
make lite-m17a   # source/design contract
make lite-m17b   # T4 lifecycle and five-profile transaction checks
make lite-m17c   # Release TPCC-like performance and operations run
make lite-m17    # aggregate M17 gate
```

The performance command writes its JSON evidence to
`/tmp/traf-lite-m17-report` by default. The report must retain the
