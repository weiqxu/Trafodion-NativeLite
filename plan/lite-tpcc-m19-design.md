...
4. The unified TransactionDB opens one bounded block cache and Bloom filter
   policy. `TRAF_LITE_BLOCK_CACHE_BYTES` controls capacity, defaults to
...
TPCC_SCALE=multi LITE_BUILD_TYPE=debug \
TPCC_ARTIFACT_DIR=/tmp/traf-lite-m19-full-tpcc \
...
`/tmp/traf-lite-m19-full-tpcc` and was run with
`TRAF_LITE_BLOCK_CACHE_BYTES=0`; cache lifecycle was separately checked
...
The cache can be disabled with `TRAF_LITE_BLOCK_CACHE_BYTES=0` without
