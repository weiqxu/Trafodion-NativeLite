...
`core/sql/regress/lite` lane. They mix portable SQL semantics with HBase
...
1. A versioned `liteLegacy/manifest.tsv` that accounts for every primary
...
have native coverage in `lite/TEST027`, `TEST028`, and `TEST029`. UPSERT
...
`lite/TEST030` through `lite/TEST034`. Composite leading-key prefixes
...
coverage is in `lite/TEST030` through `lite/TEST035`; the RocksDB SQLCI
...
publication. Native coverage is in `lite/TEST009`, `TEST021`,
...
portable suite is not yet complete.  `scripts/test-lite-legacy-convergence.sh`
...
The phase gate is executable with `make lite-m10`; it checks the
...
`liteLegacy/reprobe-m2-m6-2026-08-12.tsv`. It executed all 49 safe blocked
...
Values use the length-delimited `LTMD1` format.  The public
`LiteRocksDBStore::scanMetadataRows` API exposes a metadata table as an
...
| M4 catalog DDL and constraints | Complete for the declared Lite Storage surface | Native `TEST009`, `TEST021`, `TEST036`, `TEST037`; later metadata/DDL compatibility work and `TEST043` extend this surface. RI CASCADE and computed system columns remain outside the boundary. |
...
- Replace the process-global `LiteTxnState` singleton with a transaction
  context owned by each `ContextCli` session.
- Keep `LiteStorageManager` process-owned so one server process owns and
...
- Preserve the current `LiteTxn` executor-facing API boundary so scan and
...
`LiteTxnContext`. Transaction, tuple-flow, scan/DML, DDL, and executor-root
...
native-HBase user-transaction DDL restrictions; Lite keeps its existing
...
`make lite-m11a` exercises the static wiring plus statement, transaction,
...
server. The `make lite-m11b` lifecycle gate verifies store ownership,
...
Implemented evidence (2026-08-14): `LiteStorage.h` defines the engine,
...
`TRAF_LITE_ACTIVATION_FAULT=after-format` terminates before activation;
...
`make lite-m13` proves that rejection, activation retry, active reads,
...
explicit. `make lite-m14a` validates those assets and emits the expected
...
- Capture the existing `make lite-m10`, `make lite-m11`, and
  `make lite-m13` results before changing concurrency or isolation.

Gate: `make lite-m14a` validates the pinned inputs and produces a
...
explicit reusable prepared-statement programs. `make lite-m14c` loads a
...
Gate: `make lite-m14c` runs each profile deterministically in isolation,
...
`make lite-m14d` runs a real two-session T4 JDBC matrix covering dirty
...
Gate: `make lite-m14d` passes the versioned isolation matrix, consistency
...
`make lite-m14e` loads a smoke database, executes five repeated pairs of
...
`make lite-m14f` loads two warehouses with two districts and 100
...
Gate: `make lite-m14f` produces a reproducible non-compliant performance
...
Status: complete. `make lite-m14` has explicit M10, M11, M12, M13, and
...
`test-lite-tpcc-qualification.sh` validates the three claim checklists and
...
- Add `make lite-m14` as the aggregate M14A-M14F gate and keep M10-M13
...
`make lite-m15g` reruns M15A-M15F, builds Release, loads an explicitly
