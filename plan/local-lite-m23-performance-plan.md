...
在同一机器和同一数据规模下重跑 LocalLite 与 MySQL，固定 warmup、重复次数、
事务 mix 和报告格式；执行完整正确性、恢复和资源门禁。

提交：`test: qualify m23 tpcc performance`

验收：LocalLite TPS 至少 100、方差不超过 10%、重试率不超过 1%，核心事务
