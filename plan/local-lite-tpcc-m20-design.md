# LocalLite M20: prepared plans and server-side SQL batching
...
The reduced local-lite compiler does not expose a key-aware executor plan for
...
* Multi-statement prepared text keeps the existing protocol parameter layout
  ...
  the session-local prepared plan unless they contain a keyed local-lite
...
and descriptor ownership remain server-side; keyed local-lite predicates use
