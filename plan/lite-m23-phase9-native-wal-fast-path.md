...
- 默认启用 `native` 模式；`TRAF_LITE_PHYSICAL_WAL_MODE=legacy` 可以回退
...
- native 模式使用 `TRAF_LITE_NATIVE_WAL_FAULT=before-wal|after-canonical`
...
- `make lite-release -j2`：通过；
- `scripts/test-lite-m22a.sh`：通过；
- `scripts/test-lite-m22b.sh`：通过；
- `scripts/test-lite-tpcc-loader.sh`：在允许本机 TCP 后通过 smoke；
...
`/tmp/traf-lite-tpcc-performance.LSi2A3/`。
