# LocalLite M21: thread-affine multi-worker execution
...
qualification workload is exposed as `make local-lite-m21-tpcc`.
...
The server is still a bounded loopback LocalLite runtime. It does not claim
...
thread only owns the process default CLI context and LocalLite store lease.
...
for embedded LocalLite contexts. The LocalLite storage/OCC publication path
...
`scripts/test-local-lite-m21-concurrency.sh` proves:
...
sessions and opens the same LocalLite store only after schema creation has
closed the server. It encodes deterministic TPCC rows with the LocalLite row
codec, stages them with `LocalLiteTxn::insertRows`, and commits through the
