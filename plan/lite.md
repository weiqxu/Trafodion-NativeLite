...
[`lite-transaction-roadmap.md`](lite-transaction-roadmap.md).

## Current State

The implementation is compile-time gated by `TRAF_LITE=1` and
`-DTRAF_LITE`. The default Trafodion build remains available through
normal targets such as `make all`.

Implemented:

- Top-level `make lite` target.
...
```bash
TRAF_HOME=$(pwd)/core/sqf make -C core/sql/nskgmake TRAF_LITE=1 SQ_PHANDLE_VERIFIER=1 SQ_MTYPE=64 SQ_BTYPE=d SQ_MBTYPE=64d linuxdebug
```

Before a narrow clean rebuild, use the same ABI flags for clean and build:

```bash
TRAF_HOME=$(pwd)/core/sqf make -C core/sql/nskgmake TRAF_LITE=1 SQ_PHANDLE_VERIFIER=1 SQ_MTYPE=64 SQ_BTYPE=d SQ_MBTYPE=64d linuxdebugclean
OMPI_CXX=/usr/bin/g++ make lite
```
...
To verify persistence, use the same `TRAF_LITE_STORE_DIR` across two separate
...
`LiteTxnContext`. Connection threads may overlap, but requests enter a
...
- Loading an older `libsqlcilib.so` because `LD_LIBRARY_PATH` points at another
  build first.
- Rebuilding without `TRAF_LITE=1`.
...
## Lite Storage SQL Regress Lane

`core/sql/regress/lite/runregr` provides a native-only regression driver
...
tracked separately in `plan/lite-legacy-regress-roadmap.md`. Milestone 0
...
row IDs. Values contain a store-level row wrapper around a `LTBR1` versioned
...
- `core/sql/sqlcomp/CmpSeabaseDDLtable.cpp` implements lite
  ...
  write or remove table metadata through `LiteRocksDBStore`.
...
- [x] **Complete M15 Trafodion MVCC/OCC concurrency and Release qualification.**
  ...
  - [x] Pass `make lite-m15g` in Release mode at 32 warehouses and 32
...
- [x] **Complete M14 TPC-C-like qualification for the supported single-node
  ...
  - [x] Add `make lite-m14`; publish separate functional, TPC-C-like, and
...
- [x] **Complete M13 exclusive unified storage for restart-based
  ...
  - [x] `make lite-m13` composes M12 and the M13 SQLCI format gate.
...
- [x] **Complete M11 sessionized runtime and standalone server.**
  ...
  - [x] Transaction TCBs, tuple flow, scan/DML TCBs, DDL, and executor-root
    ...
    `LiteTxnState` singleton is removed.
  - [x] `make lite-m11a` covers independent context isolation,
    commit/rollback, same-key conflicts, and reset/destroy cleanup at the
    Lite Storage transaction boundary.
  ...
  - [x] `make lite-m11` composes the M11A, M11B, and M11C gates.

- [x] **Build RocksDB dependency detection and link flags.**
  - Implemented in `core/sql/nskgmake/Makerules.linux`.
  - Verified by `scripts/test-lite-runtime.sh` and `make lite`.

- [x] **Add native RocksDB catalog/table store module.**
  - Implemented in `core/sql/litestore/LiteRocksDBStore.h` and
    `core/sql/litestore/LiteRocksDBStore.cpp`.
...
- [x] **Route supported local DML and index operations through the bounded
  ...
  - `UPDATE`, `DELETE`, `MERGE`, `UPSERT`, and `CREATE INDEX` are implemented
    for the declared Lite Storage surface and covered by native `TEST026`-`TEST035`.
...
- [x] **Prevent old HBase access TDB build from returning `NULL`.**
  - Implemented as `LiteUnsupportedHbaseTcb` in
    `core/sql/executor/LiteStorageStubs.cpp`.

- [x] **Avoid building unsupported SQF monitor/tools targets in lite.**
  - Implemented by removing `make_monitor` and top-level `tools` from
    `LITE_COMPONENTS` in `core/sqf/Makefile`.
...
- [x] **Persist Lite table rows as versioned binary aligned payloads.**
  ...
  - Executor `INSERT INTO ... VALUES` and `INSERT ... SELECT` now evaluate the
    ...
    `LTBR1` binary aligned row data through `LiteRocksDBStore`.
...
- [x] **Harden predicates for Lite RocksDB scans.**
  - Ensure predicate-only columns are available to expression evaluation even
    when they are not part of the final projection.
  - Implemented in `core/sql/executor/LiteStorageStubs.cpp` by
    materializing the scan ascii tuple from persisted `LTBR1` rows, evaluating
...
- [x] **Extend lite scalar type coverage for small exact NUMERIC types.**
  ...
  - `NUMERIC(p,s)` values with precision `1..18` are stored as fixed binary
    executor values in the local `LTBR1` row; write-side encoding still comes
...
- [x] **Extend lite scalar type coverage for DECIMAL and BigNum NUMERIC
  ...
  - Lite Storage persists both by copying executor-produced bytes into the
    canonical `LTBR1` binary aligned row and projecting those bytes back into
    executor scan rows.
  - Runtime validation covers executor inserts and scan predicates over
...
disabled. Lite Storage code should fail explicitly if one of those paths is
reached.
...
```bash
TRAF_HOME=$(pwd)/core/sqf TRAF_LITE=1 LD_LIBRARY_PATH=$SQL_LIBS:$SQF_LIBS $SQL_LIBS/sqlci -v
...
bash scripts/test-lite-rocksdb-sqlci.sh
make lite-regress
make lite-m11
```
...
Detailed design and current validation limits: `plan/lite-tpcc-m20-design.md`.
