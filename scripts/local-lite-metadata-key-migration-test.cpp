...
void downgrade(const char *path)
{
          ...
          if (!copiedLegacyValue)
            {
              ...
              rocksdb_writebatch_put(batch, legacyKey.data(), legacyKey.size(),
  ...
  rocksdb_writebatch_delete(batch, marker.data(), marker.size());
...
void verify(const char *path)
{
  ...
  char *error = NULL;
  size_t valueLength = 0;
  char *rawValue = rocksdb_get(db, readOptions, marker.data(), marker.size(),
                               &valueLength, &error);
