...
test_root=$(mktemp -d /tmp/traf-local-lite-m21.XXXXXX)
...
env TRAF_HOME="$traf_home" TRAF_LOCAL_LITE=1 \
  TRAF_LOCAL_STORE_DIR="$capacity_store_dir" \
  LD_LIBRARY_PATH="$sql_libs:$sqf_libs:${LD_LIBRARY_PATH:-}" \
...
wait "$capacity_pid"
capacity_pid=
echo "LocalLite M21 $clients-worker session isolation and capacity checks passed"
