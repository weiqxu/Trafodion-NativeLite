...
test_root=$(mktemp -d /tmp/traf-local-lite-m14c.XXXXXX)
...
server_pid=
port=${NATIVELITE_TEST_PORT:-$((26000 + ($$ % 10000)))}
...
cleanup() {
  ...
  if [[ "$rc" -ne 0 && -s "$server_log" ]]; then
    echo "NativeLite M14C server log:" >&2
...
start_server() {
  ...
  for _ in $(seq 1 200); do
    ...
    if ! kill -0 "$server_pid" 2>/dev/null; then
      cat "$server_log" >&2
      fail "NativeLite server exited during M14C startup"
    fi
    sleep 0.05
  done
  fail "NativeLite server did not become ready for M14C"
}
...
echo "LocalLite M14C five-profile T4 JDBC transaction checks passed"
