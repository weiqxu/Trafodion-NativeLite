...
store_dir=$(mktemp -d /tmp/traf-lite-rocksdb-sqlci.XXXXXX)
...
run_sqlci_trace_scan() {
  env TRAF_HOME="$traf_home" \
    TRAF_LITE=1 \
    TRAF_LITE_TRACE_SCAN=1 \
    TRAF_LITE_TRACE_SNAPSHOT=1 \
    TRAF_LITE_STORE_DIR="$store_dir" \
    LD_LIBRARY_PATH="$sql_libs:$sqf_libs:${LD_LIBRARY_PATH:-}" \
    "$sqlci"
}
...
grep -q 'librocksdb' <<<"$ldd_output" ||
  fail "sqlci must link to librocksdb in lite"
...
grep -q 'lite table already exists' <<<"$duplicate_output" ||
...
grep -Eq '^ *1 *$' <<<"$bind_output" ||
  fail "executor scan did not return projected lite row"
grep -Eq '^ *2 *$' <<<"$bind_output" ||
  fail "executor scan did not return second projected lite row"
...
if grep -q 'does not exist or is inaccessible' <<<"$bind_output"; then
  fail "Lite catalog NATable bind reported table missing"
fi
...
grep -Eq '^one[[:space:]]+one[[:space:]]*$' <<<"$self_join_output" ||
  fail "self-join over lite table did not return expected row"
grep -q -- '--- 1 row(s) selected.' <<<"$self_join_output" ||
  fail "self-join over lite table did not report one selected row"
if grep -q 'LOCK' <<<"$self_join_output"; then
  fail "self-join over lite table hit RocksDB LOCK"
fi
self_join_snapshot_acquires=$(
  grep -c 'LITE_SNAPSHOT_ACQUIRE' <<<"$self_join_output"
)
[[ "$self_join_snapshot_acquires" -eq 1 ]] ||
  fail "self-join must acquire one statement snapshot for its shared table"
grep -q 'LITE_SNAPSHOT_REUSE' <<<"$self_join_output" ||
  fail "self-join scan TCBs did not reuse the statement snapshot"
grep -q 'LITE_SNAPSHOT_RELEASE' <<<"$self_join_output" ||
...
query_shape_scan_count=$(grep -c 'LITE_SCAN_FULL' <<<"$query_shape_output")
...
if grep -q 'LOCK' <<<"$query_shape_output"; then
  fail "query-shape lite coverage hit RocksDB LOCK"
fi
...
aggregate_expr_scan_count=$(grep -c 'LITE_SCAN_FULL' <<<"$aggregate_expr_output")
...
if grep -q 'LOCK' <<<"$aggregate_expr_output"; then
  fail "aggregate expression lite coverage hit RocksDB LOCK"
fi
...
grouped_aggregate_scan_count=$(grep -c 'LITE_SCAN_FULL' <<<"$grouped_aggregate_output")
...
if grep -q 'LOCK' <<<"$grouped_aggregate_output"; then
  fail "grouped aggregate lite coverage hit RocksDB LOCK"
fi
...
grep -q 'unsupported lite column type' <<<"$error_output" ||
...
grep -q 'rollback' <<<"$transaction_output" ||
  fail "Lite transaction did not read its own pending insert"
grep -q 'commit' <<<"$transaction_output" ||
  fail "Lite transaction commit did not persist inserted row"
grep -q -- '--- 0 row(s) selected.' <<<"$transaction_output" ||
  fail "Lite transaction rollback did not discard inserted row"
transaction_selected_count=$(grep -c -- '--- 1 row(s) selected.' <<<"$transaction_output")
[[ "$transaction_selected_count" -ge 3 ]] ||
  fail "Lite transaction scans did not report expected selected rows"
transaction_full_scan_count=$(grep -c 'LITE_SCAN_FULL' <<<"$transaction_output")
[[ "$transaction_full_scan_count" -ge 4 ]] ||
  fail "transaction SELECT and COUNT queries did not use executor full scans"
grep -q 'LITE_SNAPSHOT_REUSE' <<<"$transaction_output" ||
  fail "separate SELECT statements did not reuse the transaction snapshot"
grep -q 'LITE_SNAPSHOT_RELEASE' <<<"$transaction_output" ||
...
grep -q 'duplicate lite primary key' <<<"$atomic_primary_commit_output" ||
...
grep -q 'duplicate lite unique key' <<<"$atomic_unique_commit_output" ||
...
grep -q 'duplicate lite primary key' \
  <<<"$autocommit_atomic_values_output" ||
...
pk_duplicate_count=$(grep -c 'duplicate lite primary key' <<<"$primary_key_output")
...
unique_duplicate_count=$(grep -c 'duplicate lite unique key' <<<"$unique_key_output")
...
grep -q 'LITE_SCAN_GET_ROW' <<<"$trace_pk_output" ||
  fail "primary-key equality query did not use lite get-row scan"
...
numeric_get_row_count=$(grep -c 'LITE_SCAN_GET_ROW' <<<"$trace_numeric_pk_output")
[[ "$numeric_get_row_count" -ge 5 ]] ||
  fail "numeric/decimal/BigNum primary-key equality queries did not use lite get-row scan"
...
unique_get_row_count=$(grep -c 'LITE_SCAN_GET_ROW' <<<"$trace_unique_output")
[[ "$unique_get_row_count" -ge 7 ]] ||
  fail "unique-key equality queries did not use lite get-row scan"
...
secondary_index_scan_count=$(
  grep -c 'LITE_SCAN_INDEX_EQ' <<<"$secondary_index_trace_output"
)
[[ "$secondary_index_scan_count" -ge 5 ]] ||
  fail "secondary-index equality queries did not use lite index scans"
grep -q 'LITE_SCAN_INDEX_BOUNDED' <<<"$secondary_index_trace_output" ||
  fail "secondary-index range query did not use bounded lite index scan"
grep -q 'LITE_INDEX_BOUNDS index=IX_TRACE_CODE' <<<"$secondary_index_trace_output" ||
  fail "optimizer did not select the code secondary index"
grep -q 'LITE_INDEX_BOUNDS index=IX_TRACE_LABEL' <<<"$secondary_index_trace_output" ||
  fail "optimizer did not select the unique label secondary index"
grep -Eq 'LITE_INDEX_ONLY covering=[1-9][0-9]* base_lookups=0' <<<"$secondary_index_trace_output" ||
  fail "current secondary-index records did not use covering RocksDB values"
grep -Eq 'LITE_INDEX_BOUNDS .*candidates=2' <<<"$secondary_index_trace_output" ||
...
grep -q 'LITE_SCAN_FULL' <<<"$trace_full_output" ||
...
grep -q 'TRAFODION_SCAN' <<<"$explain_output" ||
  fail "lite explain did not include Trafodion scan operator"
...
grep -q 'key_columns: A' <<<"$explain_output" ||
  fail "lite key equality explain did not expose key column"
...
grep -q 'probes: 1' <<<"$explain_output" ||
  fail "lite key equality explain did not show single-probe access"
grep -q 'key_columns: SYSKEY' <<<"$explain_output" ||
  fail "keyless lite explain did not expose synthetic SYSKEY metadata"
...
grep -q 'lite_storage: rocksdb' <<<"$secondary_explain_output" ||
...
echo "lite RocksDB sqlci smoke passed"
