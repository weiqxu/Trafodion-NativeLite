...
work_dir=$(mktemp -d /tmp/traf-lite-store-boundary.XXXXXX)
store_dir="$work_dir/store"
ready_file="$work_dir/ready"
src="$work_dir/lite_store_boundary.cpp"
bin="$work_dir/lite_store_boundary"
holder_pid=""
...
TRAF_LITE_STORE_DIR="$store_dir" "$bin" hold "$ready_file" &
holder_pid=$!
...
for _ in $(seq 1 50); do
  ...
  if ! kill -0 "$holder_pid" >/dev/null 2>&1; then
    ...
    fail "lite boundary holder exited before ready"
  fi
  sleep 0.1
done
[[ -f "$ready_file" ]] || fail "lite boundary holder did not become ready"

set +e
boundary_output=$(TRAF_LITE_STORE_DIR="$store_dir" "$bin" open 2>&1)
boundary_status=$?
set -e

[[ "$boundary_status" -ne 0 ]] ||
  fail "second lite process unexpectedly opened the same store"
grep -q 'lite store is already open by another process' <<<"$boundary_output" ||
  fail "cross-process store diagnostic missing: $boundary_output"
grep -q 'TRAF_LITE_STORE_DIR' <<<"$boundary_output" ||
  fail "cross-process store diagnostic must name TRAF_LITE_STORE_DIR"

echo "lite store process boundary probe passed"
