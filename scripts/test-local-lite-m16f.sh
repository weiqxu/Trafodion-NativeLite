...
stock_p95=$(sed -n 's/.*"stock_level"[^}]*"p95":\([0-9]*\).*/\1/p' \
  "$workload_report" | head -1)
printf '{"contract_version":1,"milestone":"M16F","source_revision":"%s","stock_level_access":{"range_scans":"required_nonzero","point_reads":"required_nonzero","full_scans":0},"throughput_tps":%s,"stock_level_p95_us":%s,"production_target":{"throughput_tps":50,"stock_level_p95_ms":2000},"formal_tpmc_claim":"not_claimed"}\n' \
  "$(git -C "$repo_root" rev-parse HEAD)" "${throughput:-0}" "${stock_p95:-0}" \
