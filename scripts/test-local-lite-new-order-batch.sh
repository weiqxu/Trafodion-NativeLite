...
grep -q 'SELECT I_ID,I_PRICE FROM TPCC_ITEM WHERE I_ID IN (' "$source_file" ||
  fail "New-Order item batch lookup is missing"
grep -q 'SELECT S_I_ID,S_QUANTITY,S_YTD,S_ORDER_CNT,S_DIST_01' "$source_file" ||
  fail "New-Order stock batch lookup is missing"
