...
cxx=${CXX:-/usr/bin/g++}
run_root=$(mktemp -d /tmp/traf-lite-metadata-migration.XXXXXX)
trap 'rm -rf "$run_root"' EXIT

"$cxx" -std=c++11 -Wall -Wextra -Werror \
  -I"${ROCKSDB_INC_DIR:-/usr/include}" \
  "$repo_root/scripts/lite-metadata-key-migration-test.cpp" \
...
run_sqlci() {
  env TRAF_HOME="$traf_home" TRAF_LITE=1 \
    TRAF_LITE_STORE_DIR="$run_root/store" \
...
"$run_root/metadata-key-migration-test" verify \
  "$run_root/store/transactiondb"
echo "Lite metadata-key migration checks passed"
