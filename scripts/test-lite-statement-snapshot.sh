...
work_dir=$(mktemp -d /tmp/traf-lite-statement-snapshot.XXXXXX)
store_dir="$work_dir/store"
src="$work_dir/lite_statement_snapshot.cpp"
bin="$work_dir/lite_statement_snapshot"
...
int main()
{
  ...
  LiteTxn setupTxn(&setupStore, txnContext);
  ...
  LiteTxnManager::beginStatement(txnContext, &statementOwner, 7);

  LiteRocksDBStore firstScanStore;
  LiteTxn firstScan(&firstScanStore, txnContext, &statementOwner, 7);
  if (!scanCount(&firstScan, table, 1))
    return 1;

  LiteRocksDBStore writerStore;
  LiteTxn writer(&writerStore, txnContext);
  ...
  LiteTxn repeatedScan(&repeatedScanStore, txnContext, &statementOwner, 7);
  if (!scanCount(&repeatedScan, table, 1) ||
      !getFound(&repeatedScan, table, secondRowId, false))
    return 1;

  LiteTxnManager::endStatement(txnContext, &statementOwner, 7);
  LiteTxnManager::beginStatement(txnContext, &statementOwner, 8);

  LiteRocksDBStore nextExecutionStore;
  LiteTxn nextExecution(&nextExecutionStore, txnContext, &statementOwner, 8);
  if (!scanCount(&nextExecution, table, 2) ||
      !getFound(&nextExecution, table, secondRowId, true))
    return 1;

  LiteTxnManager::endStatement(txnContext, &statementOwner, 8);
  printf("lite statement snapshot probe passed\n");
  return 0;
}
CPP
