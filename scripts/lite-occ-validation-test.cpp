...
bool LiteBuildPrimaryKey(const LiteTableDef &,
                              const std::string &,
                              std::string *,
                              std::string *error)
{
...
{
public:
  Context() : value(LiteTxnManager::createContext()) {}
  ~Context() { LiteTxnManager::destroyContext(value); }
  LiteTxnContext *value;
};
...
{
  std::string error;
  if (LiteTxnManager::begin(context.value, &error)) return true;
...
{
  return LiteTxnManager::commit(context.value, error);
}

static bool update(LiteRocksDBStore *store,
                   Context &context,
                   const LiteTableDef &table,
                   uint64_t rowId,
                   const std::string &after)
{
  LiteTxn txn(store, context.value);
  LiteRow before;
  bool found = false;
  std::string error;
...
static bool insert(LiteRocksDBStore *store,
                   Context &context,
                   const LiteTableDef &table,
...
int main()
{
  ...
  setenv("TRAF_LITE_COMMIT_HOLD_MS", "100", 1);
  ...
  Context first;
  Context second;
  LiteRocksDBStore firstStore;
  LiteRocksDBStore secondStore;
  ...
  Context sameFirst;
  Context sameSecond;
  LiteRocksDBStore sameFirstStore;
  LiteRocksDBStore sameSecondStore;
  ...
  LiteRocksDBStore phantomReaderStore;
  LiteRocksDBStore phantomWriterStore;
  if (!begin(phantomReader) || !begin(phantomWriter)) return 1;
  LiteTxn scanTxn(&phantomReaderStore, phantomReader.value);
  ...
  LiteRocksDBStore missingReaderStore;
  LiteRocksDBStore missingWriterStore;
  if (!begin(missingReader) || !begin(missingWriter)) return 1;
  LiteTxn missingTxn(&missingReaderStore, missingReader.value);
  LiteRow missing;
  bool found = true;
  ...
  LiteRocksDBStore indexOverlayStore;
  if (!begin(indexOverlay)) return 1;
  LiteTxn indexTxn(&indexOverlayStore, indexOverlay.value);
  rows.clear();
  ...
  LiteOccState indexState;
  if (!LiteTxnManager::occState(indexOverlay.value, &indexState) ||
      indexState.indexRangeReads < 2 || indexState.fullScans != 0 ||
      !LiteTxnManager::rollback(indexOverlay.value, &error))
    {
  ...
  Context indexReader;
  Context indexWriter;
  LiteRocksDBStore indexReaderStore;
  LiteRocksDBStore indexWriterStore;
  if (!begin(indexReader) || !begin(indexWriter)) return 1;
  LiteTxn emptyIndexTxn(&indexReaderStore, indexReader.value);
  rows.clear();
  ...
  Context ddlReader;
  LiteRocksDBStore ddlReaderStore;
  if (!begin(ddlReader)) return 1;
  LiteTxn ddlScan(&ddlReaderStore, ddlReader.value);
  rows.clear();
  ...
  const std::string metrics = LiteOccMetricsJson();
  ...
  printf("lite Trafodion OCC validation probe passed\n");
  return 0;
}
