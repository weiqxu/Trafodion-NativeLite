...
store_dir=$(mktemp -d /tmp/traf-lite-m16d.XXXXXX)
...
run_sqlci() {
  env TRAF_HOME="$traf_home" TRAF_LITE=1 \
    TRAF_LITE_TRACE_SCAN=1 TRAF_LITE_TRACE_SNAPSHOT=1 \
...
grep -q 'LITE_SCAN_PRIMARY_RANGE table=.*TPCC_ORDER_LINE_STOCK_IX' <<<"$output" ||
...
echo "Lite M16D optimizer range and join-equivalence checks passed"
