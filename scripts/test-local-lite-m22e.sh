...
grep -q 'boundKeyInputCount_' \
  "$repo_root/core/sql/executor/LocalLiteStorageStubs.cpp" || {
  echo "FAIL: executor bound-key plan support is missing" >&2
  exit 1
}

"$repo_root/scripts/test-local-lite-t4jdbc.sh"
"$repo_root/scripts/test-local-lite-m21-concurrency.sh"
echo "LocalLite M22E bound-key and concurrent compiler gate passed"
