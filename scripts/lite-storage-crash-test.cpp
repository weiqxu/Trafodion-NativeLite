...
LiteStorageEngine *createEngine(const std::string &backend)
{
  if (backend == "rocksdb")
    return LiteCreateRocksDBTransactionEngine();
  if (backend == "sqlite")
    return LiteCreateSQLiteEngine();
  return NULL;
}
...
int main(int argc, char **argv)
{
  ...
  LiteStorageEngine *engine = createEngine(backend);
  if (!engine)
    return 2;
