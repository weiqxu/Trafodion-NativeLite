...
[[ -x "$sqlci" ]] || fail "missing built local-lite sqlci: $sqlci"

store_dir=$(mktemp -d /tmp/traf-local-lite-context-transactions.XXXXXX)
...
# executor path rather than only the local-store transaction facade.
...
echo "local-lite ContextCli transaction probe passed"
