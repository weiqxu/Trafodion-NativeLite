...
roadmap="$repo_root/plan/local-lite-transaction-roadmap.md"
design="$repo_root/plan/local-lite-m22-concurrency-qualification.md"

fail() { echo "FAIL: $*" >&2; exit 1; }

for target in 18 19 20 21 22; do
  grep -q "^local-lite-m${target}:" "$makefile" ||
    fail "Makefile is missing aggregate target local-lite-m${target}"
done
for phase in a b c d e f g h; do
  grep -q "^local-lite-m22${phase}:" "$makefile" ||
...
echo "LocalLite M22G aggregate target and evidence consistency checks passed"
