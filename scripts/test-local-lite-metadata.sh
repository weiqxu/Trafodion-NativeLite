...
[[ -x "$sqlci" ]] || {
  echo "FAIL: missing local-lite sqlci: $sqlci" >&2
  exit 2
}

store_dir=$(mktemp -d /tmp/traf-local-lite-metadata.XXXXXX)
...
output=$(
  env TRAF_HOME="$traf_home" \
    ...
    "$sqlci" <<'SQL'
CREATE TABLE LLMD_TEST (id INT NOT NULL, name VARCHAR(12), PRIMARY KEY(id));
CREATE INDEX LLMD_TEST_I ON LLMD_TEST(name);
SELECT CATALOG_NAME, SCHEMA_NAME, OBJECT_NAME, OBJECT_TYPE, OBJECT_UID
  FROM TRAFODION."_MD_".OBJECTS WHERE OBJECT_NAME = 'LLMD_TEST';
SELECT OBJECT_NAME FROM TRAFODION."_MD_".OBJECTS
  WHERE OBJECT_NAME = 'LLMD_TEST_I';
...
if grep -q '\*\*\* ERROR' <<<"$output"; then
  echo "$output" >&2
  echo "FAIL: local-lite metadata SQL returned an error" >&2
  exit 1
fi
grep -q 'LLMD_TEST' <<<"$output" || {
  echo "$output" >&2
  echo "FAIL: OBJECTS did not expose the created table" >&2
  exit 1
}
grep -q 'LLMD_TEST_I' <<<"$output" || {
  echo "$output" >&2
...
echo "local-lite metadata SQL check passed"
