...
for document in \
  ...
  "$repo_root/plan/lite.md" \
  "$repo_root/plan/lite-transaction-roadmap.md" \
  "$repo_root/plan/lite-legacy-regress-roadmap.md"; do
  grep -q 'M16' "$document" || fail "M16 is missing from $document"
done

echo "Lite M16G final evidence and documentation checks passed"
