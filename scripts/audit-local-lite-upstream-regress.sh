...
legacy_dir="$regress_root/localLiteLegacy"
...
usage() {
  cat <<'USAGE'
Usage: audit-local-lite-upstream-regress.sh [--check|--report|--list-newregr]
...
"$repo_root/scripts/audit-local-lite-legacy-regress.sh" --check >/dev/null

work_dir=$(mktemp -d "${TMPDIR:-/tmp}/traf-local-lite-upstream-audit.XXXXXX")
...
printf 'local-lite upstream regress inventory check passed: standard=%d (legacy=%d hive=%d qat=%d), newregr-paired=%d, newregr-unpaired=%d\n' \
  "$standard_count" "$legacy_count" "$hive_count" "$qat_count" \
