...
run_root=$(mktemp -d /tmp/traf-local-lite-storage-cutover.XXXXXX)
...
run_sqlci() {
  ...
  env TRAF_HOME="$traf_home" TRAF_LOCAL_LITE=1 \
    TRAF_LOCAL_STORE_DIR="$selected_store" \
...
set +e
run_sqlci "$store_root" TRAF_LOCAL_LITE_ACTIVATION_FAULT=after-format \
  <<'SQL' >/dev/null 2>&1
SELECT 1 FROM (VALUES(1)) AS T(N);
exit;
SQL
...
echo "LocalLite exclusive unified storage checks passed"
