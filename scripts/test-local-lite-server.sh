...
test_root=$(mktemp -d /tmp/traf-local-lite-server.XXXXXX)
...
set +e
env TRAF_HOME="$traf_home" TRAF_LOCAL_LITE=1 \
  TRAF_LOCAL_STORE_DIR="$test_root/other-store" \
  LD_LIBRARY_PATH="$sql_libs:$sqf_libs:${LD_LIBRARY_PATH:-}" \
...
echo "Local-lite standalone server lifecycle checks passed"
