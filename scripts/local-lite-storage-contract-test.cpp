...
void fail(const std::string &message,
          const LocalLiteStorageStatus *status = NULL)
{
...
LocalLiteStorageTxn *begin(LocalLiteStorageSession *session)
{
...
void expectValue(LocalLiteStorageTxn *txn,
...
void expectMissing(LocalLiteStorageTxn *txn, const std::string &key)
{
  LocalLiteStorageStatus status;
  std::string value;
  bool found = false;
...
int main(int argc, char **argv)
{
  ...
  LocalLiteStorageEngine *engine = NULL;
  if (std::string(argv[1]) == "rocksdb")
    engine = LocalLiteCreateRocksDBTransactionEngine();
  else if (std::string(argv[1]) == "sqlite")
    engine = LocalLiteCreateSQLiteEngine();
  else
    fail("unknown storage backend " + std::string(argv[1]));
  if (!engine->open(argv[2], options, &status))
    fail("open engine", &status);

  LocalLiteStorageSession *first = engine->createSession(&status);
  LocalLiteStorageSession *second = engine->createSession(&status);
  if (!first || !second)
  ...
  LocalLiteStorageTxn *writer = begin(first);
  ...
  LocalLiteStorageTxn *observer = begin(second);
  ...
  LocalLiteStorageTxn *reader = begin(second);
  ...
  LocalLiteStorageTxn *snapshotReader = begin(second);
  expectValue(snapshotReader, "row/t/0001", "alpha");
  LocalLiteStorageTxn *updater = begin(first);
  ...
  LocalLiteStorageTxn *lockOwner = begin(first);
  LocalLiteStorageTxn *contender = begin(second);
  ...
  if (contender->put("row/t/conflict", "contender", &status) ||
      status.code != LOCAL_LITE_STORAGE_CONFLICT || !status.retryable)
  ...
  LocalLiteStorageTxn *aborted = begin(first);
  ...
  LocalLiteStorageTxn *cancelled = begin(first);
  cancelled->cancel();
  if (cancelled->put("row/t/0003", "gamma", &status) ||
      status.code != LOCAL_LITE_STORAGE_CANCELLED)
  ...
  LocalLiteStorageMetrics metrics = engine->metrics();
  ...
  LocalLiteStorageEngine *restored = std::string(argv[1]) == "rocksdb"
      ? LocalLiteCreateRocksDBTransactionEngine()
      : LocalLiteCreateSQLiteEngine();
  options.createIfMissing = false;
  if (!restored->open(argv[5], options, &status))
    fail("open restored store", &status);
  LocalLiteStorageSession *restoredSession =
      restored->createSession(&status);
  LocalLiteStorageTxn *restoredReader = begin(restoredSession);
  ...
  LocalLiteStorageEngine *watermarked = std::string(argv[1]) == "rocksdb"
      ? LocalLiteCreateRocksDBTransactionEngine()
      : LocalLiteCreateSQLiteEngine();
  LocalLiteStorageOptions watermarkOptions;
  ...
  LocalLiteStorageSession *watermarkSession =
      watermarked->createSession(&status);
  if (!watermarkSession)
    fail("create disk-watermark session", &status);
  LocalLiteStorageTxn *rejected = watermarkSession->begin(&status);
  if (rejected || status.code != LOCAL_LITE_STORAGE_NO_SPACE)
    fail("disk watermark did not reject a new transaction", &status);
  delete watermarkSession;
  watermarked->close();
  delete watermarked;
  std::cout << "LocalLite " << argv[1]
