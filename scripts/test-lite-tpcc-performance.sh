...
if [[ -n "${TPCC_TEST_ROOT:-}" ]]; then
  ...
  test_root=$(mktemp -d /tmp/traf-lite-tpcc-performance.XXXXXX)
fi
...
start_server() {
  ...
  if [[ -n "$minimum_free" ]]; then
    env TRAF_HOME="$traf_home" TRAF_LITE=1 \
      ...
      TRAF_LITE_MINIMUM_FREE_BYTES="$minimum_free" \
      TRAF_LITE_CHECKPOINT_DIR="$checkpoint_store/transactiondb" \
      TRAF_LITE_GROUP_COMMIT_WINDOW_US="${TRAF_LITE_GROUP_COMMIT_WINDOW_US:-0}" \
...
env TRAF_HOME="$traf_home" TRAF_LITE=1 \
  TRAF_LITE_STORE_DIR="$active_store" \
  TRAF_LITE_MINIMUM_FREE_BYTES=18446744073709551615 \
  TRAF_LITE_CHECKPOINT_DIR="$checkpoint_store/transactiondb" \
  LD_LIBRARY_PATH="$sql_libs:$sqf_libs:${LD_LIBRARY_PATH:-}" \
  "$server" --listen 127.0.0.1 --port "$port" --workers "$workers" >"$server_log" 2>&1 &
server_pid=$!
...
echo "Lite M14F multi-warehouse performance and operations checks passed"
