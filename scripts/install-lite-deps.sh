...
usage() {
  cat <<'USAGE'
Usage: scripts/install-lite-deps.sh [options]

Install native OS packages needed by the Trafodion lite build.
...
Options:
  ...
  -n, --dry-run      Print commands and checks without changing the system.
      ...
      --prefix DIR   MPICH compatibility directory to create.
                     Default: core/sqf/opt/lite-mpich
...
dry_run=0
use_sudo=1
mpich_prefix="$repo_root/core/sqf/opt/lite-mpich"
...
require_confirmation() {
  ...
  printf 'Install lite native dependencies now? [y/N] '
  read -r reply
  case "$reply" in
    y|Y|yes|YES) ;;
    *) echo "Aborted."; exit 1 ;;
