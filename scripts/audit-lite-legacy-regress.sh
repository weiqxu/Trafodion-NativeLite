...
manifest=${LITE_LEGACY_MANIFEST:-$regress_root/liteLegacy/manifest.tsv}
suites=(charsets compGeneral core executor fullstack2 privs1 privs2 seabase udr)
mode=check

usage() {
  cat <<'USAGE'
Usage: audit-lite-legacy-regress.sh [--check|--report|--candidates]

Validate and summarize the versioned lite legacy regress manifest.
...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/traf-lite-legacy-audit.XXXXXX")
...
echo "lite legacy manifest check passed: $inventory_count primary TEST inputs"
