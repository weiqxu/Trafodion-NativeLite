...
work_dir=$(mktemp -d /tmp/traf-lite-session-transactions.XXXXXX)
store_dir="$work_dir/store"
src="$work_dir/lite_session_transactions.cpp"
bin="$work_dir/lite_session_transactions"
...
static bool insert(LiteTxn *txn,
                   const LiteTableDef &table,
                   const char *value)
{
  uint64_t rowId = 0;
...
int main()
{
  LiteTxnContext *sessionA = LiteTxnManager::createContext();
  LiteTxnContext *sessionB = LiteTxnManager::createContext();
  LiteTxnContext *observer = LiteTxnManager::createContext();
  ...
  if (!LiteTxnManager::beginForExecutor(sessionA, 101, &error) ||
      !LiteTxnManager::beginForExecutor(sessionB, 202, &error) ||
      LiteTxnManager::currentExecutorTxnId(sessionA) != 101 ||
      LiteTxnManager::currentExecutorTxnId(sessionB) != 202)
    {
  ...
  error.clear();
  if (LiteTxnManager::rollbackForExecutor(sessionA, 202, &error) ||
      error.find("transaction context mismatch") == std::string::npos ||
      !LiteTxnManager::active(sessionA) ||
      !LiteTxnManager::active(sessionB))
    {
      fprintf(stderr, "cross-session transaction ID was accepted: %s\n",
              error.c_str());
      return 1;
    }
  if (!LiteTxnManager::rollbackForExecutor(sessionA, 101, &error) ||
      !LiteTxnManager::rollbackForExecutor(sessionB, 202, &error))
    {
  ...
  LiteRocksDBStore storeA;
  LiteRocksDBStore storeB;
  LiteRocksDBStore observerStore;
  LiteTxn txnA(&storeA, sessionA);
  LiteTxn txnB(&storeB, sessionB);
  LiteTxn observerTxn(&observerStore, observer);
  ...
  if (!LiteTxnManager::begin(sessionA, &error) ||
      !LiteTxnManager::begin(sessionB, &error) ||
    ...
    return 1;
  if (!LiteTxnManager::commit(sessionA, &error))
    {
      fprintf(stderr, "first session commit failed: %s\n", error.c_str());
      return 1;
    }
  error.clear();
  if (LiteTxnManager::commit(sessionB, &error) ||
  ...
  if (!LiteTxnManager::begin(sessionA, &error) ||
      ...
      !LiteTxnManager::rollback(sessionA, &error) ||
      !LiteTxnManager::commit(sessionB, &error) ||
  ...
  if (!LiteTxnManager::begin(sessionB, &error) ||
      !insert(&txnB, table, "discard-on-reset"))
    return 1;
  LiteTxnManager::beginStatement(sessionB, &resetStatement, 31);
  LiteTxn resetScan(&storeB, sessionB, &resetStatement, 31);
  if (!scanValues(&resetScan, table, 3, "discard-on-reset", NULL))
    return 1;
  LiteTxnManager::resetContext(sessionB);
  if (LiteTxnManager::active(sessionB) ||
  ...
  if (!LiteTxnManager::begin(sessionA, &error) ||
    ...
    return 1;
  LiteTxnManager::beginStatement(sessionA, &destroyStatement, 41);
  LiteTxn destroyScan(&storeA, sessionA, &destroyStatement, 41);
  if (!scanValues(&destroyScan, table, 4, "discard-on-destroy", NULL))
    return 1;
  LiteTxnManager::destroyContext(sessionA);
  sessionA = NULL;

  LiteTxnContext *replacement = LiteTxnManager::createContext();
  LiteRocksDBStore replacementStore;
  LiteTxn replacementTxn(&replacementStore, replacement);
  if (!scanValues(&observerTxn, table, 3, "after-reset", "discard-on-destroy") ||
      !LiteTxnManager::begin(replacement, &error) ||
      !insert(&replacementTxn, table, "discard-on-destroy") ||
      !LiteTxnManager::commit(replacement, &error) ||
  ...
  LiteTxnManager::destroyContext(replacement);
  LiteTxnManager::destroyContext(sessionB);
  LiteTxnManager::destroyContext(observer);
  printf("lite session transaction probe passed\n");
  return 0;
}
CPP
