...
run_root=$(mktemp -d /tmp/traf-lite-storage-backends.XXXXXX)
...
"$cxx" -std=c++11 -O2 -Wall -Wextra -Werror -pthread \
  ...
  "$repo_root/scripts/lite-storage-benchmark.cpp" \
...
"$cxx" -std=c++11 -O2 -Wall -Wextra -Werror -pthread \
  ...
  "$repo_root/core/sql/litestore/LiteStorage.cpp" \
  "$repo_root/core/sql/litestore/LiteStorageSQLite.cpp" \
  "$repo_root/scripts/lite-storage-crash-test.cpp" \
  -L"${ROCKSDB_LIB_DIR:-/usr/lib/x86_64-linux-gnu}" \
...
for backend in rocksdb sqlite; do
  ...
  set +e
  TRAF_LITE_STORAGE_FAULT=before-commit \
  ...
  set +e
  TRAF_LITE_STORAGE_FAULT=after-commit \
    "$run_root/storage-crash-test" "$backend" fault-commit "$store_path"
  after_rc=$?
  set -e
  [[ $after_rc -eq 87 ]] || {
  ...
  for operation in checkpoint backup; do
    set +e
    TRAF_LITE_STORAGE_FAULT=$operation \
...
transactions=${LITE_M12_BENCHMARK_TXNS:-300}
...
# on-disk tooling, and the lowest-risk migration from the current Lite
