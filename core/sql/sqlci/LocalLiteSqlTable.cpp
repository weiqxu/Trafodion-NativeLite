                                         ...
                                         const std::string &object)
{
  if (!localLiteLegacyOutput())
    return reportError(env, "local-lite sequence does not exist");
...
static std::vector<std::string> localLiteWrapLegacyDDLLine(
...
static std::vector<std::string> localLiteLegacyColumnLines(
    const LocalLiteTableDef &table, size_t columnIndex, bool first)
{
  if (columnIndex >= table.columns.size())
    return std::vector<std::string>();

  const LocalLiteColumnDef &column = table.columns[columnIndex];
  bool identity = column.defaultClass == COM_IDENTITY_GENERATED_BY_DEFAULT ||
                  column.defaultClass == COM_IDENTITY_GENERATED_ALWAYS;
  if (!identity)
    return localLiteWrapLegacyDDLLine(
        localLiteLegacyColumnLine(column, first));
  ...
  return localLiteWrapLegacyDDLLine(line);
}
  ...
  for (size_t i = 0; i < table.columns.size(); i++)
    {
      std::vector<std::string> columnLines =
          localLiteLegacyColumnLines(table, i, i == 0);
...
bool LocalLiteSqlTable_process(const char *sqlText, SqlciEnv *sqlciEnv, short *retcode)
{
  ...
  // diagnostic retrieval, not a local catalog metadata request; let the
  ...
  if (startsWithWord(sql, "SHOWDDL") || startsWithWord(sql, "SHOWSTATS"))
    {
      ...
      if (!showStats && restUpper.compare(0, strlen("SEQUENCE"), "SEQUENCE") == 0 &&
           ...
           isspace(static_cast<unsigned char>(restUpper[strlen("SEQUENCE")]))))
        {
          ...
          if (!parsed || !loaded)
            *retcode = reportError(sqlciEnv, error.empty()
                ? "invalid local-lite sequence name" : error);
          else if (!found)
