...
static bool loadLibrary(LocalLiteRocksDBStore *store, const std::string &id,
                        std::string *path, std::string *language, std::string *error)
{
  std::string c, s, n; if (!objectName(id, &c, &s, &n)) { *error = "invalid local-lite UDR library"; return false; }
  std::string value; bool found = false;
  if (!store->loadCatalogRecord(libraryKey(c, s, n), &value, &found, error)) return false;
  if (!found) { *error = "local-lite UDR library does not exist: " + id; return false; }
  if (value.compare(0, 6, "LLUL1\n") != 0) { *error = "invalid local-lite UDR library metadata"; return false; }
...
static bool loadRoutine(LocalLiteRocksDBStore *store, const std::string &id,
                        UdrDef *def, std::string *error)
{
  std::string c, s, n; if (!objectName(id, &c, &s, &n)) { *error = "invalid local-lite UDR name"; return false; }
  std::string key = "udr|" + c + "|" + s + "|" + n, value; bool found = false;
  if (!store->loadCatalogRecord(key, &value, &found, error)) return false;
  if (!found) { *error = "local-lite UDR does not exist: " + id; return false; }
  if (!decodeUdr(value, key, def)) { *error = "invalid local-lite UDR metadata: " + id; return false; }
  return true;
}
...
static bool parseRoutineDDL(const std::string &sql, UdrDef *d, std::string *error)
{
  ...
  if (nameEnd == std::string::npos || !objectName(rest.substr(0, nameEnd), &d->catalog, &d->schema, &d->name)) { *error = "invalid local-lite UDR name"; return true; }
  size_t open = rest.find('(', nameEnd); if (open == std::string::npos) { *error = "local-lite UDR parameters are required"; return true; }
  size_t close = matchingParen(rest, open); if (close == std::string::npos) { *error = "unterminated local-lite UDR parameter list"; return true; }
  ...
  if (d->kind == UDR_FUNCTION || d->kind == UDR_TABLE_FUNCTION)
    {
      ...
      if (returns == std::string::npos || (language != std::string::npos && returns > language)) { *error = "local-lite UDR RETURNS clause is required"; return true; }
      ...
      if (upper(r).compare(0, 5, "TABLE") == 0)
        { size_t ro = r.find('('); size_t rc = ro == std::string::npos ? std::string::npos : matchingParen(r, ro); if (ro == std::string::npos || rc == std::string::npos) { *error = "invalid local-lite table UDR result columns"; return true; } r = r.substr(ro + 1, rc - ro - 1); }
      else if (r.size() && r[0] != '(')
        {
  ...
  if (language == std::string::npos) { *error = "local-lite UDR LANGUAGE clause is required"; return true; }
  ...
  size_t ext = tailUpper.find("EXTERNAL NAME"); if (ext == std::string::npos) { *error = "local-lite UDR EXTERNAL NAME clause is required"; return true; }
  size_t exStart = ext + strlen("EXTERNAL NAME"); d->symbol = unquote(token(tail, &exStart));
  size_t lib = tailUpper.find("LIBRARY", exStart); if (lib == std::string::npos) { *error = "local-lite UDR LIBRARY clause is required"; return true; }
...
static bool loadDefLibrary(LocalLiteRocksDBStore *store, UdrDef *d, std::string *path, std::string *error)
{
...
static bool invokeNative(const UdrDef &d, const std::string &path,
                         ...
                         std::string *error)
{
  if (args.size() > 2 || d.returns.size() > 2) { *error = "local-lite native UDR supports at most two INT inputs and two outputs"; return false; }
  for (size_t i = 0; i < args.size(); i++) if (args[i].kind != Value::INT && args[i].kind != Value::NULLVAL) { *error = "local-lite native UDR currently accepts INT arguments only"; return false; }
  for (size_t i = 0; i < d.returns.size(); i++) if (upper(d.returns[i].type).find("INT") == std::string::npos) { *error = "local-lite native UDR currently returns INT values only"; return false; }
  void *handle = path == "builtin" ? dlopen(NULL, RTLD_NOW | RTLD_GLOBAL) : dlopen(path.c_str(), RTLD_NOW | RTLD_LOCAL);
  if (!handle) { *error = "unable to load local-lite UDR library " + path + ": " + dlerror(); return false; }
  void *symbol = d.symbol.empty() ? NULL : dlsym(handle, d.symbol.c_str());
  if (!symbol) { *error = "unable to resolve local-lite UDR symbol " + d.symbol; if (path != "builtin") dlclose(handle); return false; }
  ...
  if (rc != SQLUDR_SUCCESS) { *error = std::string("local-lite UDR failed") + (msg[0] ? ": " + std::string(msg) : ""); return false; }
                       ...
                       std::string *error)
{
  ...
  for (size_t i = 0; i < args.size(); i++) { if (args[i].kind != Value::INT) { *error = "local-lite Java UDR accepts INT arguments only"; return false; } cmd << " " << args[i].i; }
  FILE *pipe = popen(cmd.str().c_str(), "r"); if (!pipe) { *error = "unable to launch local-lite Java UDR"; return false; }
  char buf[512] = ""; std::string output; while (fgets(buf, sizeof(buf), pipe)) output += buf; int status = pclose(pipe);
  if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) { *error = "local-lite Java UDR failed"; return false; }
  std::vector<std::string> values = splitTop(trim(output)); out->clear();
  for (size_t i = 0; i < d.returns.size(); i++) { Value v; if (i >= values.size() || !parseValue(values[i], &v)) { *error = "invalid local-lite Java UDR result"; return false; } out->push_back(v); }
  return true;
}
...
static bool invoke(const UdrDef &d, LocalLiteRocksDBStore *store,
                   ...
                   std::vector<Value> *out, std::string *error)
{
  ...
  if (argCount != inputCount) { *error = "local-lite UDR argument count mismatch"; return false; }
  std::vector<Value> args; for (size_t i = 0; i < argCount; i++) { Value v; if (!parseValue(argTexts[i], &v)) { *error = "invalid local-lite UDR argument"; return false; } args.push_back(v); }
...
// Built-in routines make the local-lite regression deterministic while the
// same invocation path also supports dlopen() libraries and Java adapters.
extern "C" SQLUDR_INT32 local_lite_udr_add2(SQLUDR_INT32 *a, SQLUDR_INT32 *b, SQLUDR_INT32 *r, SQLUDR_INT16 *ai, SQLUDR_INT16 *bi, SQLUDR_INT16 *ri, SQLUDR_CHAR *state, SQLUDR_CHAR *msg, SQLUDR_INT32 call, SQLUDR_STATEAREA *, SQLUDR_UDRINFO *)
...
extern "C" SQLUDR_INT32 local_lite_udr_swap2(SQLUDR_INT32 *a, SQLUDR_INT32 *b, SQLUDR_INT32 *r1, SQLUDR_INT32 *r2, SQLUDR_INT16 *ai, SQLUDR_INT16 *bi, SQLUDR_INT16 *i1, SQLUDR_INT16 *i2, SQLUDR_CHAR *, SQLUDR_CHAR *, SQLUDR_INT32 call, SQLUDR_STATEAREA *, SQLUDR_UDRINFO *)
...
extern "C" SQLUDR_INT32 local_lite_udr_fail(SQLUDR_INT32 *, SQLUDR_INT32 *, SQLUDR_INT32 *, SQLUDR_INT16 *, SQLUDR_INT16 *, SQLUDR_INT16 *, SQLUDR_CHAR *, SQLUDR_CHAR *msg, SQLUDR_INT32 call, SQLUDR_STATEAREA *, SQLUDR_UDRINFO *)
{ if (call != SQLUDR_CALLTYPE_FINAL) strcpy(msg, "intentional local-lite UDR failure"); return call == SQLUDR_CALLTYPE_FINAL ? SQLUDR_SUCCESS : SQLUDR_ERROR; }
...
static bool handleDDL(const std::string &sql, SqlciEnv *env, short *retcode, std::string *error)
{
  ...
  if (starts(sql, "CREATE LIBRARY"))
    {
      if (!root) { *error = "only DB__ROOT may create local-lite UDR libraries"; return true; }
      std::string rest = trim(sql.substr(strlen("CREATE LIBRARY"))); size_t end = rest.find_first_of(" \t\r\n"); std::string c,s,n;
      if (end == std::string::npos || !objectName(rest.substr(0,end), &c,&s,&n)) { *error = "invalid local-lite UDR library name"; return true; }
      size_t file = upper(rest).find("FILE", end); if (file == std::string::npos) { *error = "local-lite UDR library FILE clause is required"; return true; }
      size_t o = file + 4; std::string path = unquote(token(rest, &o)); std::string value = "LLUL1\nC\n" + escapeField(path) + "\n";
      if (!store.storeCatalogRecord(libraryKey(c,s,n), value, error)) return true; *retcode = 0; return true;
    }
  if (starts(sql, "DROP LIBRARY"))
    {
      if (!root) { *error = "only DB__ROOT may drop local-lite UDR libraries"; return true; }
      std::string c,s,n; std::string rest = trim(sql.substr(strlen("DROP LIBRARY"))); if (!objectName(rest,&c,&s,&n)) { *error = "invalid local-lite UDR library name"; return true; }
  ...
  if (starts(sql, "CREATE FUNCTION") || starts(sql, "CREATE PROCEDURE") || starts(sql, "CREATE TABLE MAPPING FUNCTION"))
    {
      if (!root) { *error = "only DB__ROOT may create local-lite UDR routines"; return true; }
      UdrDef d; if (!parseRoutineDDL(sql, &d, error)) return false; if (!error->empty()) return true;
      d.owner = "DB__ROOT"; std::string path, language;
      std::string lc, ls, ln;
      if (!objectName(d.library, &lc, &ls, &ln)) { *error = "invalid local-lite UDR library name"; return true; }
      d.library = lc + "." + ls + "." + ln;
  ...
  if (starts(sql, "DROP FUNCTION") || starts(sql, "DROP PROCEDURE") || starts(sql, "DROP TABLE MAPPING FUNCTION"))
    {
      if (!root) { *error = "only DB__ROOT may drop local-lite UDR routines"; return true; }
      ...
      if (!objectName(trim(sql.substr(p)),&c,&s,&n)) { *error = "invalid local-lite UDR name"; return true; }
...
static bool runInvocation(const std::string &sql, SqlciEnv *env, short *retcode)
{
  ...
  LocalLiteRocksDBStore store; UdrDef d; std::string error; if (!loadRoutine(&store,name,&d,&error)) { *retcode = fail(env,&error,error); writeLine(env,"*** ERROR[local-lite] "+error); return true; }
  std::vector<Value> values; if (!invoke(d,&store,args,&values,&error)) { *retcode = fail(env,&error,error); writeLine(env,"*** ERROR[local-lite] "+error); return true; }
...
bool LocalLiteUdr_process(const char *sqlText, SqlciEnv *env, short *retcode)
{
  ...
  std::string error;
  if (handleDDL(sql, env, retcode, &error)) { if (!error.empty()) *retcode = fail(env,&error,error), writeLine(env,"*** ERROR[local-lite] "+error); return true; }
  if (runInvocation(sql, env, retcode)) return true;
  return false;
}

bool LocalLiteUdr_prepare(const char *sqlText, const char *statementName,
...
bool LocalLiteUdr_executePrepared(const char *statementName, SqlciEnv *env, short *retcode)
{
