...
void clearSQLiteStatus(LocalLiteStorageStatus *status)
{
  if (!status)
    return;
...
void setSQLiteStatus(LocalLiteStorageStatus *status,
                 ...
                 LocalLiteStorageStatus *status)
{
  ...
  LocalLiteStorageCode storageCode = LOCAL_LITE_STORAGE_IO_ERROR;
  bool retryable = false;
  if (code == SQLITE_BUSY || code == SQLITE_LOCKED)
    {
      storageCode = LOCAL_LITE_STORAGE_CONFLICT;
      retryable = true;
    }
  else if (code == SQLITE_INTERRUPT)
    storageCode = LOCAL_LITE_STORAGE_CANCELLED;
  else if (code == SQLITE_FULL)
    storageCode = LOCAL_LITE_STORAGE_NO_SPACE;
  else if (code == SQLITE_CORRUPT || code == SQLITE_NOTADB)
    storageCode = LOCAL_LITE_STORAGE_CORRUPTION;
  else if (code == SQLITE_MISUSE || code == SQLITE_RANGE)
    storageCode = LOCAL_LITE_STORAGE_INVALID;
...
class SQLiteCursor : public LocalLiteStorageCursor
{
public:
...
class SQLiteSession : public LocalLiteStorageSession
{
public:
...
class SQLiteEngine : public LocalLiteStorageEngine
{
...
public:
                    ...
                    LocalLiteStorageStatus *status)
  {
    if (opened_ || path.empty())
      {
        setSQLiteStatus(status, LOCAL_LITE_STORAGE_INVALID,
    ...
    if (!ok)
      {
        if (code != SQLITE_ROW)
          return checkSQLite(code, admin_, "run SQLite integrity check",
                             status);
        setSQLiteStatus(status, LOCAL_LITE_STORAGE_CORRUPTION,
...
private:
  ...
  bool exec(sqlite3 *db, const char *sql,
            LocalLiteStorageStatus *status) const
  {
    char *message = NULL;
  ...
  bool checkDiskWatermark(LocalLiteStorageStatus *status) const
  {
    ...
    if (statvfs(parent.c_str(), &info) != 0)
      {
        setSQLiteStatus(status, LOCAL_LITE_STORAGE_IO_ERROR,
    ...
    if (available < options_.minimumFreeBytes)
      {
        setSQLiteStatus(status, LOCAL_LITE_STORAGE_NO_SPACE,
...
class SQLiteTransaction : public LocalLiteStorageTxn
{
public:
...
private:
  bool checkActive(LocalLiteStorageStatus *status) const
  {
    ...
    if (cancelled_)
      {
        setSQLiteStatus(status, LOCAL_LITE_STORAGE_CANCELLED,
...
LocalLiteStorageTxn *SQLiteSession::begin(LocalLiteStorageStatus *status)
{
...
bool SQLiteCursor::next(LocalLiteStorageRecord *record,
                        bool *end,
                        LocalLiteStorageStatus *status)
{
  if (!record || !end)
