...
void clearStatus(LiteStorageStatus *status)
{
  if (!status)
    return;
...
void setStatus(LiteStorageStatus *status,
...
LiteStorageCode classifyRocksError(const std::string &message,
                                        bool *retryable)
{
  *retryable = false;
                       ...
                       LiteStorageStatus *status)
{
  ...
  rocksdb_free(error);
  bool retryable = false;
  const LiteStorageCode code = classifyRocksError(message, &retryable);
...
class RocksDBCursor : public LiteStorageCursor
{
public:
...
class RocksDBSession : public LiteStorageSession
{
public:
...
class RocksDBTransactionEngine : public LiteStorageEngine
{
...
public:
                    ...
                    LiteStorageStatus *status)
  {
    if (opened_)
      {
        setStatus(status, LITE_STORAGE_INVALID,
...
private:
  ...
  bool checkDiskWatermark(LiteStorageStatus *status) const
  {
    ...
    if (statvfs(path_.c_str(), &info) != 0)
      {
        setStatus(status, LITE_STORAGE_IO_ERROR,
    ...
    if (available < options_.minimumFreeBytes)
      {
        setStatus(status, LITE_STORAGE_NO_SPACE,
...
class RocksDBTransaction : public LiteStorageTxn
{
public:
...
private:
  bool checkActive(LiteStorageStatus *status) const
  {
    ...
    if (cancelled_)
      {
        setStatus(status, LITE_STORAGE_CANCELLED,
...
LiteStorageTxn *RocksDBSession::begin(LiteStorageStatus *status)
{
...
bool RocksDBCursor::next(LiteStorageRecord *record,
                         bool *end,
                         LiteStorageStatus *status)
{
  if (!record || !end)
