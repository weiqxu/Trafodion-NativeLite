...
static const char LITE_BINARY_ROW_MAGIC[] = "LTBR1";
...
static bool isVariableType(LiteCodecType type)
{
...
static size_t typeAlignment(LiteCodecType type)
{
  switch (type)
    {
    case LL_TYPE_INT8:
...
static bool computeLayout(const LiteTableDef &table,
                          std::vector<LiteStoredColumn> *columns,
                          size_t *rowLen,
                          std::string *error)
{
  columns->clear();
  ...
  for (size_t i = 0; i < table.columns.size(); i++)
    {
      LiteStoredColumn &col = (*columns)[i];
      if (!mapType(table.columns[i].type, &col.type, &col.length,
                   ...
                   &col.intervalEnd))
        {
          setError(error, "unsupported lite binary row column type: " +
                              ...
                              std::string *error)
{
  ...
  if (!parseScaledDigits(value, col.scale, col.precision,
                         &negative, &digits))
    {
      setError(error, "invalid lite decimal literal");
      return false;
    }
  if (digits.size() != col.length)
    {
      setError(error, "lite decimal storage length mismatch");
      return false;
    }
                             ...
                             std::string *error)
{
  ...
  if (!parseScaledDigits(value, col.scale, col.precision,
                         &negative, &digits))
    {
      setError(error, "invalid lite BigNum literal");
      return false;
    }
                       ...
                       std::string *error)
{
  ...
  if (isNullValue(value) && !col.nullable)
    {
      setError(error, "NULL is not allowed for lite NOT NULL column");
      return false;
    }
  if (col.nullable)
    ...
    case LL_TYPE_NUMERIC:
      {
        ...
        if (!parseScaledInt64(value, col.scale, &scaled))
          {
            setError(error, "invalid lite numeric literal");
            return false;
          }
        switch (col.length)
          {
          ...
          default:
            setError(error, "lite numeric key requires binary numeric storage");
            return false;
          }
      }
    case LL_TYPE_DATETIME:
      {
        ...
        if (!datetime.isValid())
          {
            setError(error, "invalid lite datetime literal");
            return false;
          }
        ...
        if (datetime.getValueLen() < baseLength ||
          ...
          {
            setError(error, "invalid lite datetime storage");
            return false;
          }
    ...
    case LL_TYPE_BOOLEAN:
      {
        ...
        if (v < 0)
          {
            setError(error, "invalid lite BOOLEAN literal");
            return false;
          }
        str_cpy_all(target, reinterpret_cast<char *>(&v), sizeof(v));
        return true;
    ...
    case LL_TYPE_INTERVAL:
      {
        ...
        if (!interval.isValid() || interval.getValueLen() > col.length)
          {
            setError(error, "invalid lite interval literal");
            return false;
          }
...
static bool refreshLiteDivisionColumns(
    const LiteTableDef &table,
    const std::vector<LiteStoredColumn> &columns,
    char *row,
    std::string *error)
{
  for (size_t division = 0; division < table.columns.size(); division++)
    {
      const LiteColumnDef &definition = table.columns[division];
                              ...
                              std::string *error)
{
  if (fields.size() != table.columns.size())
    {
      ...
      snprintf(message, sizeof(message),
               "lite row field count does not match %s (%lu vs %lu)",
  ...
  for (size_t i = 0; i < columns.size(); i++)
    {
      const LiteStoredColumn &col = columns[i];
  ...
  if (!refreshLiteDivisionColumns(table, columns, &row[0], error))
    return false;
  ...
  encoded->assign(LITE_BINARY_ROW_MAGIC,
                  sizeof(LITE_BINARY_ROW_MAGIC) - 1);
  encoded->append(row);
  return true;
}
                            ...
                            std::string *error)
{
  if (!row && rowLen > 0)
    {
      setError(error, "missing lite binary row");
      return false;
    }
  encoded->assign(LITE_BINARY_ROW_MAGIC,
                  sizeof(LITE_BINARY_ROW_MAGIC) - 1);
...
static bool initializeCanonicalRow(const std::vector<LiteStoredColumn> &columns,
                                   char *row,
                                   size_t rowLen)
{
...
static bool copyAttrToCanonical(const char *srcRow,
                                ...
                                ExpTupleDesc *srcTd,
                                char *destRow,
                                const LiteStoredColumn &destCol,
                                std::string *error)
{
  if (!srcAttr)
    {
      setError(error, "lite insert missing source attribute");
      return false;
    }
  ...
  if (srcAttr->getVCIndicatorLength() > 0)
    {
      ...
      if (srcAttr->getOffset() == ExpOffsetMax &&
          !hasRange(srcRowLen, srcAttr->getVoaOffset(),
                    ExpVoaSize))
        {
          setError(error, "truncated lite executor row VOA");
          return false;
        }
      ...
      if (!hasRange(srcRowLen, srcOffset,
                    srcAttr->getVCIndicatorLength()))
        {
          setError(error, "truncated lite executor row");
          return false;
        }
                                 ...
                                 std::string *error)
{
  if (!srcTd || !srcRow)
    {
      setError(error, "lite insert missing executor row");
      return false;
    }
  UInt32 sourceOffset = 0;
  if (table.primaryKeyColumns.empty() &&
      srcTd->numAttrs() == table.columns.size() + 1)
    // Storage allocates SYSKEY, so LTBR1 contains catalog user columns only.
    sourceOffset = 1;
  else if (srcTd->numAttrs() != table.columns.size())
    {
      setError(error, "lite insert row does not match table column count");
      return false;
    }
                              ...
                              std::string *error)
{
  if (!isNull || columnIndex >= table.columns.size() ||
      encoded.size() < sizeof(LITE_BINARY_ROW_MAGIC) - 1 ||
      encoded.compare(0, sizeof(LITE_BINARY_ROW_MAGIC) - 1,
                      LITE_BINARY_ROW_MAGIC) != 0)
    {
      setError(error, "invalid lite binary row");
      return false;
    }
  ...
  if (!computeLayout(table, &columns, &rowLen, error))
    return false;
  const char *row = encoded.data() + sizeof(LITE_BINARY_ROW_MAGIC) - 1;
  size_t available = encoded.size() - (sizeof(LITE_BINARY_ROW_MAGIC) - 1);
  const LiteStoredColumn &column = columns[columnIndex];
  if (!column.nullable)
    {
      *isNull = false;
      return true;
  ...
  if (!hasRange(available, column.nullIndOffset, 1))
    {
      setError(error, "truncated lite binary row null bitmap");
      return false;
    }
  *isNull = ExpTupleDesc::isNullValue(
    ...
    std::string *error)
{
  if (!srcTd || !srcRow || !encoded)
    {
      setError(error, "lite update missing executor row");
      return false;
    }
  if (srcTd->numAttrs() != updatedColumnIndexes.size())
    {
      setError(error,
               "lite update row does not match updated column count");
      return false;
    }
  if (original.size() < sizeof(LITE_BINARY_ROW_MAGIC) - 1 ||
      memcmp(original.data(), LITE_BINARY_ROW_MAGIC,
             sizeof(LITE_BINARY_ROW_MAGIC) - 1) != 0)
    {
      setError(error, "invalid lite binary row payload");
      return false;
    }
  ...
  std::string row =
    original.substr(sizeof(LITE_BINARY_ROW_MAGIC) - 1);
  if (row.size() < fullRowLen)
    {
      setError(error, "truncated lite binary row payload");
      return false;
    }
  ...
  for (UInt32 i = 0; i < srcTd->numAttrs(); i++)
    {
      ...
      if (columnIndex >= columns.size())
        {
          setError(error, "lite updated column index out of range");
          return false;
        }
...
bool LiteRebuildBinaryRow(
    ...
    std::string *error)
{
  const size_t magicLen = sizeof(LITE_BINARY_ROW_MAGIC) - 1;
  if (!encoded || newToOldColumn.size() != newTable.columns.size() ||
      addedValues.size() != newTable.columns.size())
    {
      setError(error, "invalid lite ALTER row mapping");
      return false;
    }
  if (original.size() < magicLen ||
      memcmp(original.data(), LITE_BINARY_ROW_MAGIC, magicLen) != 0)
    {
      setError(error, "invalid lite binary row payload");
      return false;
    }

  std::vector<LiteStoredColumn> oldColumns;
  std::vector<LiteStoredColumn> newColumns;
  size_t oldRowLen = 0;
  size_t newRowLen = 0;
  ...
  for (size_t i = 0; i < newColumns.size(); i++)
    {
      ...
      if (static_cast<size_t>(oldIndex) >= oldColumns.size())
        {
          setError(error, "lite ALTER row mapping is out of range");
          return false;
        }
      const LiteStoredColumn &src = oldColumns[oldIndex];
      const LiteStoredColumn &dest = newColumns[i];
      if (src.type != dest.type || src.length != dest.length ||
          src.precision != dest.precision || src.scale != dest.scale)
        {
          setError(error,
                   "lite ALTER COLUMN type conversion is not supported");
          return false;
        }
      ...
      if (isNull)
        {
          if (!dest.nullable)
            {
              setError(error,
                       "NULL is not allowed for lite NOT NULL column");
              return false;
            }
      ...
      if (isVariableType(src.type))
        {
          ...
          if (len > src.length)
            {
              setError(error, "invalid lite VARCHAR row payload");
              return false;
            }
  ...
  encoded->assign(LITE_BINARY_ROW_MAGIC, magicLen);
                              ...
                              std::string *error)
{
  if (!key)
    {
      setError(error, "missing lite primary key output");
      return false;
    }
  if (table.primaryKeyColumns.empty())
    {
      setError(error, "lite table has no primary key");
      return false;
    }
  if (encoded.size() < sizeof(LITE_BINARY_ROW_MAGIC) - 1 ||
      memcmp(encoded.data(), LITE_BINARY_ROW_MAGIC,
             sizeof(LITE_BINARY_ROW_MAGIC) - 1) != 0)
    {
      setError(error, "invalid lite binary row payload");
      return false;
  ...
  std::string storedRow =
    encoded.substr(sizeof(LITE_BINARY_ROW_MAGIC) - 1);
  ...
  for (size_t i = 0; i < table.primaryKeyColumns.size(); i++)
    {
      ...
      if (sourceIndex >= columns.size())
        {
          setError(error, "lite primary key column index out of range");
          return false;
        }

      const LiteStoredColumn &col = columns[sourceIndex];
      if (storedColumnIsNull(storedRow, col))
        {
          setError(error, "NULL is not allowed for lite primary key column");
          return false;
        }
    ...
    std::string *error)
{
  if (keyFields.size() != table.primaryKeyColumns.size())
    {
      setError(error, "lite primary key field count mismatch");
      return false;
    }
  ...
  encoded.assign(LITE_BINARY_ROW_MAGIC,
                 sizeof(LITE_BINARY_ROW_MAGIC) - 1);
  encoded.append(row);
  return LiteBuildPrimaryKey(table, encoded, key, error);
}
    ...
    std::string *error)
{
  if (!keyTd || !keyRow ||
    ...
    {
      setError(error, "lite executor primary key tuple mismatch");
      return false;
    }
  std::vector<LiteStoredColumn> columns;
  size_t rowLen = 0;
  ...
  UInt32 adjustedLen = ExpAlignedFormat::adjustDataLength(
      ...
      ExpAlignedFormat::ALIGNMENT, TRUE);
  row.resize(adjustedLen);
  std::string encoded(LITE_BINARY_ROW_MAGIC,
                      sizeof(LITE_BINARY_ROW_MAGIC) - 1);
  encoded.append(row);
  return LiteBuildPrimaryKey(table, encoded, key, error);
}
    ...
    std::string *error)
{
  if (!keyPrefix || leadingKeyFields.empty() ||
    ...
    {
      setError(error, "invalid lite primary key prefix fields");
      return false;
    }
  std::vector<LiteStoredColumn> columns;
  size_t rowLen = 0;
                             ...
                             std::string *error)
{
  if (!key || !hasKey)
    {
      setError(error, "missing lite unique key output");
      return false;
    }
  if (keyColumns.empty())
    {
      setError(error, "lite unique key has no columns");
      return false;
    }
  if (encoded.size() < sizeof(LITE_BINARY_ROW_MAGIC) - 1 ||
      memcmp(encoded.data(), LITE_BINARY_ROW_MAGIC,
  ...
  for (size_t i = 0; i < keyColumns.size(); i++)
    {
      ...
      if (sourceIndex >= columns.size())
        {
          setError(error, "lite unique key column index out of range");
          return false;
        }
    ...
    std::string *error)
{
  if (keyFields.size() != keyColumns.size())
    {
      setError(error, "lite unique key field count mismatch");
      return false;
    }
  ...
  encoded.assign(LITE_BINARY_ROW_MAGIC,
                 sizeof(LITE_BINARY_ROW_MAGIC) - 1);
  encoded.append(row);
  return LiteBuildUniqueKey(table, encoded, keyColumns, keyOrdinal,
                                 key, hasKey, error);
}

bool LiteBuildConstraintKey(
    const LiteTableDef &table, const std::string &encoded,
    const std::vector<size_t> &keyColumns, std::string *key,
    bool *hasKey, std::string *error)
{
  if (!key || !hasKey || keyColumns.empty())
    {
      setError(error, "invalid lite constraint key output");
      return false;
    }
  if (encoded.size() < sizeof(LITE_BINARY_ROW_MAGIC) - 1 ||
      memcmp(encoded.data(), LITE_BINARY_ROW_MAGIC,
  ...
  for (size_t i = 0; i < keyColumns.size(); i++)
    {
      ...
      if (sourceIndex >= columns.size())
        {
          setError(error, "lite constraint column index out of range");
          return false;
        }
      const LiteStoredColumn &col = columns[sourceIndex];
      if (storedColumnIsNull(storedRow, col))
    ...
    std::string *error)
{
  if (!payloadPrefix)
    {
      setError(error, "missing lite secondary index prefix output");
      return false;
    }
  if (keyColumns.empty() || leadingKeyFields.empty() ||
      leadingKeyFields.size() > keyColumns.size())
    {
      setError(error, "invalid lite secondary index prefix fields");
      return false;
    }
  ...
  for (size_t i = 0; i < leadingKeyFields.size(); i++)
    {
      ...
      if (columnIndex >= columns.size())
        {
          setError(error, "lite secondary index column out of range");
          return false;
        }
...
static bool orderedIndexColumnSupported(const LiteStoredColumn &column)
{
  switch (column.type)
    {
    case LL_TYPE_INT8:
  ...
  if (column.decimalStorage)
    {
      if (!hasRange(storedRow, column.offset, column.length) ||
        ...
        {
          setError(error, "invalid lite ordered decimal value");
          return false;
        }
  ...
  else
    {
      if (!hasRange(storedRow, column.offset, column.length) ||
          column.precision <= 18)
        {
          setError(error, "invalid lite ordered BigNum value");
          return false;
        }
  ...
  for (size_t i = 0; i < digits.size(); i++)
    {
      ...
      if (digit < '0' || digit > '9')
        {
          setError(error, "invalid lite ordered decimal digit");
          return false;
        }
      if (negative)
                                  ...
                                  std::string *error)
{
  if (!hasRange(storedRow, column.offset, column.length) ||
    ...
    {
      setError(error, "invalid lite ordered datetime value");
      return false;
    }
    ...
    std::string *error)
{
  ...
  if (!orderedIndexColumnSupported(column))
    {
      setError(error, "unsupported ordered lite index column type");
      return false;
    }
  ...
  if (isVariableType(column.type) || column.type == LL_TYPE_CHAR ||
      column.type == LL_TYPE_BINARY)
    {
      ...
      if (!hasRange(storedRow, column.offset, len))
        {
          setError(error, "truncated lite ordered index value");
          return false;
        }
           ...
           column.type == LL_TYPE_FLOAT64)
    {
      if (!hasRange(storedRow, column.offset, column.length) ||
        ...
        {
          setError(error, "invalid lite ordered floating index value");
          return false;
        }
      uint64_t bits = 0;
  ...
  else
    {
      if (!hasRange(storedRow, column.offset, column.length) ||
        ...
        {
          setError(error, "invalid lite ordered numeric index value");
          return false;
        }
      uint64_t bits = 0;
      memcpy(&bits, storedRow.data() + column.offset, column.length);
    ...
    std::string *error)
{
  if (!payload || !hasKey || !containsNull ||
      encodedRow.size() < sizeof(LITE_BINARY_ROW_MAGIC) - 1 ||
      memcmp(encodedRow.data(), LITE_BINARY_ROW_MAGIC,
             sizeof(LITE_BINARY_ROW_MAGIC) - 1) != 0)
    {
      setError(error, "invalid lite ordered index input");
      return false;
    }
  std::vector<LiteStoredColumn> columns;
  size_t rowLen = 0;
  if (!computeLayout(table, &columns, &rowLen, error))
    return false;
  std::string storedRow =
      encodedRow.substr(sizeof(LITE_BINARY_ROW_MAGIC) - 1);
  payload->clear();
  *hasKey = true;
  *containsNull = false;
    ...
    std::string *error)
{
  if (!payloadPrefix || leadingKeyFields.empty() ||
    ...
    {
      setError(error, "invalid lite ordered index prefix fields");
      return false;
    }
  std::vector<LiteStoredColumn> columns;
  size_t rowLen = 0;
    ...
    std::string *error)
{
  if (!payloadPrefix ||
      leadingKeyFields.size() >= index.keyColumns.size())
    {
      setError(error, "invalid lite ordered NULL index prefix");
      return false;
    }
  std::vector<LiteStoredColumn> columns;
  size_t rowLen = 0;
    ...
    std::string *error)
{
  if (!payloadPrefix || leadingKeyFields.empty() ||
      ...
      leadingKeyFields.size() > index.keyColumns.size())
    {
      setError(error, "invalid lite ordered nullable index prefix");
      return false;
    }
  std::vector<LiteStoredColumn> columns;
  size_t rowLen = 0;
                             ...
                             std::string *error)
{
  ...
  if (dest->getVCIndicatorLength() > 0)
    {
      if (!isVariableType(source.type) && source.type != LL_TYPE_CHAR &&
        ...
        {
          setError(error, "lite binary projection type mismatch");
          return false;
        }
      size_t len = 0;
      const char *src = NULL;
  ...
  switch (dest->getDatatype())
    ...
    default:
      setError(error, "unsupported lite binary projection column type");
      return false;
    }
}
                               ...
                               std::string *error)
{
  if (!destTd || !destRow)
    {
      setError(error, "lite binary projection missing destination row");
      return false;
    }
  if (encoded.size() < sizeof(LITE_BINARY_ROW_MAGIC) - 1 ||
      memcmp(encoded.data(), LITE_BINARY_ROW_MAGIC,
  ...
  if (sourceIndexes.size() != destTd->numAttrs())
    {
      setError(error, "lite binary projection does not match tuple descriptor");
      return false;
    }
  ...
  for (UInt32 i = 0; i < destTd->numAttrs(); i++)
    {
      ...
      if (table.primaryKeyColumns.empty() &&
          sourceIndex == columns.size())
        {
          if (!dest || dest->getLength() < sizeof(Int64))
            {
              setError(error, "lite SYSKEY destination is invalid");
              return false;
            }
      ...
      // Logical lite UNIQUE indexes are scanned from the base row.
      ...
      if (sourceIndex >= columns.size())
        {
          ...
          snprintf(msg, sizeof(msg),
                   "lite binary projection source index out of range: %lu of %lu",
