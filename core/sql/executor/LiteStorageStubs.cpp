...
static LiteTxnContext *liteTxnContext(ExExeStmtGlobals *globals)
{
  return globals && globals->getContext()
      ? globals->getContext()->getLiteTxnContext() : NULL;
}

static std::mutex liteScanRowMutex;
static std::map<std::string, std::deque<LiteRow> >
  liteScanRows;

static Lng32 liteStorageDiagCode(const std::string &message)
{
  if (getenv("TEST_SCHEMA_NAME") != NULL)
    {
      if (message.find("duplicate lite") != std::string::npos)
        return -8102;
...
static std::string liteScanRowKey(const char *tableName,
...
static void liteRememberScanRow(const std::string &key,
                                     const LiteRow &row)
{
  std::lock_guard<std::mutex> lock(liteScanRowMutex);
  liteScanRows[key].push_back(row);
}

static bool liteTakeScanRow(const std::string &key, LiteRow *row)
{
  std::lock_guard<std::mutex> lock(liteScanRowMutex);
  std::map<std::string, std::deque<LiteRow> >::iterator it =
    liteScanRows.find(key);
  if (it == liteScanRows.end() || it->second.empty())
    return false;
  *row = it->second.front();
  it->second.pop_front();
  if (it->second.empty())
    liteScanRows.erase(it);
  return true;
}

static void liteForgetScanRows(const std::string &key)
{
  std::lock_guard<std::mutex> lock(liteScanRowMutex);
  liteScanRows.erase(key);
}

static void liteTraceScan(const char *path, const char *tableName)
{
  const char *trace = getenv("TRAF_LITE_TRACE_SCAN");
  if (!trace || !trace[0])
    return;
  fprintf(stderr, "LITE_SCAN_%s table=%s\n",
          path ? path : "UNKNOWN",
          tableName ? tableName : "");
}

static bool liteRuntimePrimaryRow(
    bool uniqueKeyInfo,
    ex_expr *rowIdExpr,
    ExpTupleDesc *keyTd,
    ...
    std::string *error)
{
  ...
  *found = false;
  ...
  if (rowIdExpr->eval(inputAtp, workAtp) == ex_expr::EXPR_ERROR)
    {
      *error = "lite runtime primary key evaluation failed";
      return false;
    }
  ...
  if (!LiteBuildPrimaryKeyFromExecutorTuple(
...
static bool liteStaticPrimaryRows(
    Queue *getRows,
    const LiteTableDef &table,
    LiteTxn *txn,
    bool *handled,
    ...
    std::string *error)
{
  *handled = false;
  ...
  for (Lng32 index = 0; index < rowIds->numEntries(); index++)
    {
      ...
      if (!raw || strncmp(raw, "LTPK1:", 6) != 0)
        return true;
      const char *hex = raw + 6;
      const size_t hexLen = strlen(hex);
      if ((hexLen % 2) != 0)
        {
          *error = "invalid lite static primary key length";
          return false;
        }
      std::string storageKey;
      ...
      for (size_t offset = 0; offset < hexLen; offset += 2)
        {
          ...
          if (hi < 0 || hi > 15 || lo < 0 || lo > 15)
            {
              *error = "invalid lite static primary key";
              return false;
            }
  ...
  std::vector<LiteRow> resolvedRows;
...
extern "C" const char *trafLiteUnsupportedStorage()
{
  return "HDFS, Hive, and HBase are not supported in lite builds";
}

class LiteUnsupportedHbaseTcb : public ex_tcb
{
public:
  LiteUnsupportedHbaseTcb(const ComTdbHbaseAccess &tdb, ex_globals *globals)
    : ex_tcb(tdb, 1, globals)
  {
    allocateParentQueues(qparent_);
  }

  ~LiteUnsupportedHbaseTcb()
  {
    freeResources();
  }
        ...
        if (down->downState.request != ex_queue::GET_NOMORE)
          {
            ...
            *diags << DgSqlCode(-EXE_INTERNAL_ERROR)
                   << DgString0(trafLiteUnsupportedStorage());
...
static std::string liteTrim(const std::string &s)
{
  size_t b = 0;
...
static std::string liteUnquoteIdent(const std::string &s)
{
  std::string t = liteTrim(s);
  if (t.size() >= 2 && t[0] == '"' && t[t.size() - 1] == '"')
    return t.substr(1, t.size() - 2);
  return t;
}

static std::vector<std::string> liteSplitDottedName(const char *name)
{
  ...
  for (size_t i = 0; p[i]; i++)
    {
      ...
      if (c == '.' && !quoted)
        {
          parts.push_back(liteUnquoteIdent(curr));
          curr.clear();
        }
      else
        curr += c;
...
static void liteTableNameParts(const char *name,
                                    std::string *catalog,
                                    std::string *schema,
                                    std::string *object)
{
  std::vector<std::string> parts = liteSplitDottedName(name);
  *catalog = "TRAFODION";
  *schema = "SEABASE";
  object->clear();
  if (parts.empty())
...
class LiteRocksdbScanTcb : public ex_tcb
{
public:
  LiteRocksdbScanTcb(const ComTdbLiteRocksdbScan &tdb,
                          ex_globals *globals)
    : ex_tcb(tdb, 1, globals),
      qparent_(),
      pool_(NULL),
  ...
  void freeResources()
  {
    liteForgetScanRows(liteScanRowKey(
...
private:
  const ComTdbLiteRocksdbScan &scanTdb() const
  {
    return static_cast<const ComTdbLiteRocksdbScan &>(tdb);
  }

  ComTdbLiteRocksdbScan &scanTdb()
  {
    return const_cast<ComTdbLiteRocksdbScan &>(
        static_cast<const ComTdbLiteRocksdbScan &>(tdb));
  }
  ...
  bool loadRows(std::string *error)
  {
    ...
    std::string schema;
    std::string object;
    liteTableNameParts(scanTdb().getTableName(),
                            &catalog, &schema, &object);
    if (object.empty())
      {
        *error = "invalid lite table name";
        return false;
      }
    ...
    LiteTxn txn(&store_, liteTxnContext(statementGlobals),
                     statementGlobals,
    ...
    if (scanTdb().boundKeyInputCount_ > 0)
      {
        if (loadRuntimePrimaryGet(&txn, error))
          {
            liteTraceScan("RUNTIME_GET_ROW", scanTdb().getTableName());
            return true;
          }
        if (!error->empty())
          return false;
    ...
    if (handledGetRows)
      {
        ...
        liteTraceScan(scanPath, scanTdb().getTableName());
        return true;
      }
    ...
    liteTraceScan("FULL", scanTdb().getTableName());
    return txn.scanRows(table_, &rows_, error);
  }

  bool loadRuntimePrimaryGet(LiteTxn *txn, std::string *error)
  {
    ...
    if (scanTdb().boundKeyInputCount_ > 0)
      for (CollIndex i = 0; i < keyTd->numAttrs() &&
                            i < scanTdb().boundKeyInputCount_; i++)
        {
          ...
          if (getenv("TRAF_LITE_TRACE_SCAN"))
            fprintf(stderr,
                    "LITE_BOUND_INPUT key_col=%u tupp=%u offset=%u "
    ...
    if (getenv("TRAF_LITE_TRACE_SCAN"))
      {
        fprintf(stderr, "LITE_BOUND_KEY table=%s key=",
    ...
    if (!txn->getRowByKey(table_, storageKey, &row, &found, error))
      return false;
    if (getenv("TRAF_LITE_TRACE_SCAN"))
      fprintf(stderr, "LITE_BOUND_RESULT table=%s found=%d\n",
                   ...
                   std::string *error)
  {
    ...
    *handledPrimaryRange = false;
    ...
    for (Lng32 i = 0; i < getRows->numEntries(); i++)
      {
        LiteGetRowsImage *hgr =
          static_cast<LiteGetRowsImage *>(getRows->getNext());
        if (!hgr || !hgr->rowIds() || hgr->rowIds()->numEntries() == 0)
          {
            *error = "lite get row id list is empty";
            return false;
          }
        ...
        for (Lng32 j = 0; j < rowIds->numEntries(); j++)
          {
            ...
            if (rawKey &&
                (strncmp(rawKey, "LTIB1:", 6) == 0 ||
                 strncmp(rawKey, "LTPB1:", 6) == 0))
              {
                std::string bounds(rawKey + 6);
                size_t separator = bounds.find(':');
                if (separator == std::string::npos)
                  {
                    *error = "invalid lite secondary index bounds";
                    return false;
                  }
                std::string startKey;
                std::string endKey;
            ...
            if (rawKey &&
                (strncmp(rawKey, "LTIX1:", 6) == 0 ||
                 strncmp(rawKey, "LTIR1:", 6) == 0))
              {
                if (strncmp(rawKey, "LTIR1:", 6) == 0)
                  *handledIndexRange = true;
                std::string prefix;
            ...
            if (!decodeGetRowKey(rawKey, &storageKey, error))
              return false;
            if (!isLiteStorageKey(storageKey))
              return true;
            storageKeys.push_back(storageKey);
          }
      }
    ...
    if (!primaryRanges.empty())
      {
        if (!storageKeys.empty() || !indexPrefixes.empty() ||
          ...
          {
            *error = "invalid lite mixed primary range lookup";
            return false;
          }
    ...
    if (!indexPrefixes.empty() || !indexRanges.empty())
      {
        if (!storageKeys.empty() ||
          ...
          {
            *error = "invalid lite mixed secondary index lookup";
            return false;
          }
                       ...
                       std::string *error)
  {
    if (!rawKey)
      {
        *error = "lite get row id is null";
        return false;
      }

    if (strncmp(rawKey, "LTPK1:", 6) == 0)
                          ...
                          std::string *error)
  {
    ...
    if ((len % 2) != 0)
      {
        *error = "invalid lite hex get row id length";
        return false;
      }
    ...
    for (size_t i = 0; i < len; i += 2)
      {
        ...
        if (hi < 0 || lo < 0)
          {
            *error = "invalid lite hex get row id";
            return false;
          }
        storageKey->push_back(static_cast<char>((hi << 4) | lo));
      }
  ...
  bool isLiteStorageKey(const std::string &storageKey)
  {
  ...
  bool formatAndEvaluate(const LiteRow &row,
                         Lng32 *formattedLen,
                         bool *pass,
                         std::string *error)
  {
    ...
    if (!asciiTd || !convertTd)
      {
        *error = "lite missing scan tuple descriptor";
        return false;
      }
    ...
    if (!LiteProjectBinaryRow(table_, row.value, row.rowId,
                                   asciiSourceIndexes,
                                   asciiTd, asciiRow_,
    ...
    if (scanTdb().scanExpr_ && scanTdb().boundKeyInputCount_ == 0)
      {
        ...
        if (evalRetCode == ex_expr::EXPR_ERROR)
          {
            *error = "lite scan predicate evaluation failed";
            return false;
          }
    ...
    *pass = true;
    ...
    // parent. Lite Storage still projects variable columns into that row, but the
    ...
    for (UInt32 i = 0; i < td->numAttrs(); i++)
      {
        ...
        switch (attr->getDatatype())
          ...
          default:
            *error = "unsupported lite scan column type";
            return false;
          }
      }
        ...
        for (UInt32 i = 0; i < td->numAttrs(); i++)
          {
            ...
            if (!decodeFetchedColumnIndex((char *)fetched->get(i),
                                          &sourceIndex))
              {
                *error = "lite scan projection does not match fetched columns";
                return false;
              }
    ...
    if (!up->getDiagsArea())
      {
        ComDiagsArea *diags =
          ComDiagsArea::allocate(getGlobals()->getDefaultHeap());
        Lng32 code = liteStorageDiagCode(message);
                         ...
                         short *rc)
  {
    ...
    liteRememberScanRow(liteScanRowKey(
        scanTdb().getTableName(), getGlobals()->castToExExeStmtGlobals()),
        sourceRow);
  ...
  LiteTableDef table_;
  std::vector<LiteRow> rows_;
};

class LiteHbaseInsertTcb : public ex_tcb
{
public:
  LiteHbaseInsertTcb(const ComTdbHbaseAccess &tdb, ex_globals *globals)
    : ex_tcb(tdb, 1, globals),
      qparent_(),
      pool_(NULL),
      workAtp_(NULL),
  ...
  Int32 numChildren() const { return 0; }
  const ex_tcb *getChild(Int32) const { return NULL; }
  NABoolean isLiteInsert() const { return TRUE; }
...
private:
  ...
  bool loadTable(std::string *error)
  {
    ...
    std::string schema;
    std::string object;
    liteTableNameParts(insertTdb().getTableName(),
                            &catalog, &schema, &object);
    if (object.empty())
      {
        *error = "invalid lite table name";
  ...
  bool evaluateAndInsert(ex_queue_entry *down, std::string *error)
  {
    if (!workAtp_ || !convertRow_ || insertTdb().convertRowLen_ <= 0)
      {
        *error = "lite insert missing executor row buffer";
        return false;
      }
    ...
    if (insertTdb().convertExpr_)
      {
        ...
        if (evalRetCode == ex_expr::EXPR_ERROR)
          {
            *error = "lite insert expression evaluation failed";
            return false;
          }
      }
    ...
    if (!LiteNormalizeBinaryRow(table_, convertTd, convertRow_,
...
class LiteHbaseDeleteTcb : public ex_tcb
{
public:
  LiteHbaseDeleteTcb(const ComTdbHbaseAccess &tdb, ex_globals *globals)
    : ex_tcb(tdb, 1, globals),
      qparent_(),
      pool_(NULL),
      workAtp_(NULL),
  ...
  ~LiteHbaseDeleteTcb() { freeResources(); }

  NABoolean isLiteDelete() const { return TRUE; }
...
private:
                     ...
                     std::string *error) const
  {
    ...
    if (!td)
      {
        *error = "lite delete missing tuple descriptor";
        return false;
      }
        ...
        for (UInt32 i = 0; i < td->numAttrs(); i++)
          {
            ...
            if (!decodeColumnIndex(static_cast<char *>(names->get(i)),
                                   &index))
              {
                *error = "lite delete column mapping is invalid";
                return false;
              }
  ...
  bool loadTable(std::string *error)
  {
    ...
    std::string schema;
    std::string object;
    liteTableNameParts(deleteTdb().getTableName(),
                            &catalog, &schema, &object);
    if (object.empty())
      {
        *error = "invalid lite table name";
  ...
  bool evaluateAndDelete(std::string *error)
  {
    if (!workAtp_ || !asciiRow_ || !convertRow_ ||
      ...
      {
        *error = "lite delete missing executor row buffers";
        return false;
      }
    if (!loadTable(error))
      return false;
    ...
    LiteTxn txn(&store_, liteTxnContext(statementGlobals),
                     statementGlobals,
                     statementGlobals->getExecutionCount());
    std::vector<LiteRow> sourceRows;
    LiteRow sourceRow;
    if (liteTakeScanRow(liteScanRowKey(
    ...
    else
      {
        ...
        if (getenv("TRAF_LITE_TRACE_SCAN"))
          fprintf(stderr,
                  "LITE_DML_KEY table=%s kind=delete unique=%d subset=%d rowexpr=%d getrows=%d\n",
        ...
        if (!handled && !liteRuntimePrimaryRow(
                ...
                deleteTdb().liteBoundKeyInputCount_,
                deleteTdb().liteBoundKeyInputTuppIndex_,
                deleteTdb().liteBoundKeyInputOffset_,
                deleteTdb().liteBoundKeyInputLength_,
            ...
            if (found)
              {
                ...
                if (rc == ex_expr::EXPR_ERROR)
                  {
                    *error = "lite delete source conversion failed";
                    return false;
                  }
            ...
            if (deleteTdb().scanExpr_)
              {
                ...
                if (rc == ex_expr::EXPR_ERROR)
                  {
                    *error = "lite delete predicate evaluation failed";
                    return false;
                  }
    ...
    if (deleteTdb().returnRow())
      {
        ...
        const LiteRow &returnedSource = sourceRows.back();
        workAtp_->getTupp(deleteTdb().asciiTuppIndex_)
          .setDataPointer(asciiRow_);
        workAtp_->getTupp(deleteTdb().convertTuppIndex_)
          .setDataPointer(convertRow_);
        ...
        if (deleteTdb().convertExpr_->eval(
            ...
            ex_expr::EXPR_ERROR)
          {
            *error = "lite delete return-row conversion failed";
            return false;
          }
      }
...
class LiteHbaseUpdateTcb : public ex_tcb
{
public:
  LiteHbaseUpdateTcb(const ComTdbHbaseAccess &tdb, ex_globals *globals)
    : ex_tcb(tdb, 1, globals),
      qparent_(),
      pool_(NULL),
      workAtp_(NULL),
  ...
  ~LiteHbaseUpdateTcb() { freeResources(); }

  NABoolean isLiteUpdate() const { return TRUE; }
...
private:
                     ...
                     std::string *error) const
  {
    ...
    if (!td)
      {
        *error = "lite update missing tuple descriptor";
        return false;
      }
    if (names && names->numEntries() >= td->numAttrs())
      {
        for (UInt32 i = 0; i < td->numAttrs(); i++)
          {
            ...
            if (!decodeColumnIndex(static_cast<char *>(names->get(i)),
                                   &index))
              {
                *error = "lite update column mapping is invalid";
                return false;
              }
            indexes->push_back(index);
          }
  ...
  bool loadTable(std::string *error)
  {
    ...
    std::string schema;
    std::string object;
    liteTableNameParts(updateTdb().getTableName(),
                            &catalog, &schema, &object);
    if (object.empty())
      {
        *error = "invalid lite table name";
  ...
  bool evaluateAndUpdate(std::string *error)
  {
    ...
    if (!workAtp_ || !asciiRow_ || !convertRow_ ||
      ...
      {
        *error = "lite update missing executor row buffers";
        return false;
      }
    if (!loadTable(error))
      return false;
    ...
    if (!downCri || downCri->noTuples() <= 0)
      {
        *error = "lite update missing source-row descriptor";
        return false;
      }
    std::vector<LiteRow> sourceRows;
    LiteRow sourceRow;
    ...
    if (!isMerge &&
        liteTakeScanRow(liteScanRowKey(
    ...
    else
      {
        ...
        if (getenv("TRAF_LITE_TRACE_SCAN"))
          fprintf(stderr,
                  "LITE_DML_KEY table=%s kind=update unique=%d subset=%d rowexpr=%d getrows=%d\n",
        ...
        if (!isMerge && !liteStaticPrimaryRows(
        ...
        if (!isMerge && !handled &&
            !liteRuntimePrimaryRow(
                ...
                downEntry()->getAtp(), rowIdRow_, rowIdAsciiRow_,
                updateTdb().liteBoundKeyInputCount_,
                updateTdb().liteBoundKeyInputTuppIndex_,
                updateTdb().liteBoundKeyInputOffset_,
                updateTdb().liteBoundKeyInputLength_,
                table_, &txn, &handled, &sourceRow, &found, error))
          return false;
        if (handled && sourceRows.empty())
          {
            if (found)
              {
                ...
                if (rc == ex_expr::EXPR_ERROR)
                  {
                    *error = "lite update source conversion failed";
                    return false;
                  }
            ...
            if (updateTdb().scanExpr_)
              {
                ...
                if (fallbackRc == ex_expr::EXPR_ERROR)
                  {
                    *error = "lite update predicate evaluation failed";
                    return false;
                  }
    ...
    if (isMerge && sourceRows.size() > 1)
      {
        *error = "lite MERGE source matched more than one target row";
        return false;
      }
    ...
    std::vector<LiteRowMutation> mutations;
    for (size_t sourceIndex = 0;
         sourceIndex < sourceRows.size(); sourceIndex++)
      {
        ...
        if (isMerge && updateTdb().mergeUpdScanExpr_)
          {
            ...
            if (rc == ex_expr::EXPR_ERROR)
              {
                *error = "lite MERGE update predicate evaluation failed";
                return false;
              }
            if (rc == ex_expr::EXPR_FALSE)
              continue;
        ...
        if (rc == ex_expr::EXPR_ERROR)
          {
            *error = "lite update expression evaluation failed";
            return false;
          }
        if (getenv("TRAF_LITE_TRACE_SCAN"))
          fprintf(stderr, "LITE_UPDATE_BOUND count=%u attrs=%u\n",
                  static_cast<unsigned>(
                      updateTdb().liteBoundUpdateInputCount_),
                  static_cast<unsigned>(updatedTd->numAttrs()));
        for (CollIndex i = 0;
             i < updatedTd->numAttrs() &&
             i < updateTdb().liteBoundUpdateInputCount_; i++)
          {
            Attributes *attr = updatedTd->getAttr(i);
            const UInt16 tuppIndex =
              updateTdb().liteBoundUpdateInputTuppIndex_[i];
            if (!attr || updateTdb().liteBoundUpdateInputLength_[i] == 0 ||
            ...
            const UInt32 copyLength = MINOF(
                attr->getLength(),
                updateTdb().liteBoundUpdateInputLength_[i]);
            str_cpy_all(
                updateRow_ + attr->getOffset(),
                source + updateTdb().liteBoundUpdateInputOffset_[i],
                copyLength);
            if (getenv("TRAF_LITE_TRACE_SCAN"))
              fprintf(stderr,
                      "LITE_UPDATE_INPUT col=%u tupp=%u offset=%u "
                      "length=%u first=%02x\n",
                      static_cast<unsigned>(i),
                      static_cast<unsigned>(tuppIndex),
                      static_cast<unsigned>(
                          updateTdb().liteBoundUpdateInputOffset_[i]),
                      static_cast<unsigned>(copyLength),
                      static_cast<unsigned char>(source[
                          updateTdb().liteBoundUpdateInputOffset_[i]]));
            if (getenv("TRAF_LITE_TRACE_SCAN"))
              fprintf(stderr,
                      "LITE_UPDATE_TARGET col=%u table_col=%u "
                      "offset=%u first=%02x\n",
                      static_cast<unsigned>(i),
  ...
  bool evaluateMergeInsert(LiteTxn *txn, std::string *error)
  {
    if (!updateTdb().mergeInsertExpr_)
      return true;
    if (!txn || !mergeInsertRow_ || updateTdb().mergeInsertRowLen_ == 0)
      {
        *error = "lite MERGE missing insert row buffer";
        return false;
      }
    ...
    if (!insertTd)
      {
        *error = "lite MERGE missing insert tuple descriptor";
        return false;
      }
    ...
    if (rc == ex_expr::EXPR_ERROR)
      {
        *error = "lite MERGE insert expression evaluation failed";
        return false;
      }
    if (rowLen == 0)
    ...
    std::string encodedRow;
    if (!LiteNormalizeBinaryRow(table_, insertTd, mergeInsertRow_,
                                     static_cast<size_t>(rowLen),
                                     &encodedRow, error))
      return false;
    uint64_t rowId = 0;
...
ex_tcb *LiteRocksdbScanTdb::build(ex_globals *globals)
{
  LiteRocksdbScanTcb *tcb =
    new(globals->getSpace()) LiteRocksdbScanTcb(*this, globals);
...
ex_tcb *ExHbaseAccessTdb::build(ex_globals *globals)
{
  if (getAccessType() == ComTdbHbaseAccess::INSERT_ ||
    ...
    {
      LiteHbaseInsertTcb *tcb =
        new(globals->getSpace()) LiteHbaseInsertTcb(*this, globals);
      tcb->registerSubtasks();
      return tcb;
    }
  ...
  if (getAccessType() == ComTdbHbaseAccess::UPDATE_)
    {
      LiteHbaseUpdateTcb *tcb =
        new(globals->getSpace()) LiteHbaseUpdateTcb(*this, globals);
      tcb->registerSubtasks();
      return tcb;
    }
  ...
  if (getAccessType() == ComTdbHbaseAccess::DELETE_)
    {
      LiteHbaseDeleteTcb *tcb =
        new(globals->getSpace()) LiteHbaseDeleteTcb(*this, globals);
      tcb->registerSubtasks();
      return tcb;
    }

  LiteUnsupportedHbaseTcb *tcb =
    new(globals->getSpace()) LiteUnsupportedHbaseTcb(*this, globals);
  tcb->registerSubtasks();
  return tcb;
}
...
{
  errCode_ = errCode;
  errCodeStr_ = trafLiteUnsupportedStorage();
  errMethodName_ = errMethodName;
  errDetail_ = trafLiteUnsupportedStorage();
  return FALSE;
}
